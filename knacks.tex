...
They gain +1 to their Combat or Projectiles Bonus when using that weapon.
...
The character can choose to gain \glspl{fatigue} instead of spending \gls{mp} when casting a spell, even while still having plenty of \gls{mp}.
Normally, characters can cast a spell by gaining \glspl{fatigue} only if they have insufficient \gls{mp} for that spell.
...
All ritual spells cast with this focus grant a bonus to the caster's chosen casting Attribute score for the purpose of casting spells equal to half the number of Knacks the character has (rounded up).
