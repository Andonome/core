...
\begin{exampletext}
  ...
  The first \gls{pc} has a +5 Bonus, and +1 from the Knack: Supporter.
  The next has +3 (so they add half, for a +2 to the roll).
  The third has a +2~Bonus (so they add a quarter, for +1 to the roll).

  With the total Bonus at +11, the troupe make this \pgls{restingaction}; the first die is set to \dicef{6}, and they roll the second.
\end{exampletext}
