...
This book has some serious problems, and that's fine.  I've put this under a share-alike licence,\footnote{Specifically, it's under the Creative Commons Attribution-ShareAlike 4.0 International licence.} so anyone can grab a copy of the basic \LaTeX \ document it's written in and change things.  This isn't the Open Gaming Licence of D20 where they magnanimously allow you to use their word for a mechanic and let you publish things for their products -- this is something new.

No longer do imaginative \acrshortpl{gm} have to scribble their inspired house rules onto the back of an old banking statement and cellotape it to the last page of the core book.  Instead, you have the complete source code, and can modify it as you please, creating a cohesive book.  If you spot an error, you can correct it.  If you want to add a couple of spells, it's no problem.  Just download the source code from gitlab.com/andonome/first-blood, download a Latex editor, and make the changes you want.  Once you're happy with your changes, you might even send it off to a printing shop for a cheaply bound book all of your own.
...
Like so many roleplayers, I grew up with Advanced Dungeons \& Dragons. I loved the idea of a story formula -- of a set of basic principles which could be expressed in infinite worlds and ways; the promise of trying, failing, falling back and trying again -- of persistently growing power and an eternally sustainable do or die attitude.

The rules were a great at first -- I probably owe more of my maths education to A,D\&D than school. There I was at two am, rather stoned and trying to calculate healing each of my players for $1D6 \times 10\%$ of their lost HP. But like many others I grew dissatisfied with those rules before long -- the problems are well documented. Like a few others, I decided to have my own go at them. This isn't the first retroclone, and won't be the last. This is possibly also not a retroclone as it deviates in many ways from tradition, but it is an attempt to recapture some of that old feeling of `I kick the door down. What do I see?'. Though of course, a particular slant is taken for everything -- every gaming system must prioritise one thing or another. Let me run through a couple of the main aims here.

Firstly, this is a book for people who already know what an RPG is. We will not be spending a lot of time explaining what a `hitpoint' is or creating lengthy examples of how to use a Skill. It works well enough for new players so long as the Games Master knows what's what ahead of time.

Secondly, speed. I'm an impatient person, this is an RPG for impatient people. I'm keen on streamlining things with parsimonious Maths. Most tasks can be finished in a single roll. You can record most information by shuffling some coins around or ticking boxes.

Lastly, the rules attempt to present real options.  There are decisions to be made during combat which will  change players' chances of success or failure, and a defined system for characters' backstories and how they want them introduced to the game.

If all that sounds like your type of thing, your chances of liking this book are good.
...
%    \includegraphics[width=1.7in]{images/Boris_Pecikozic/the_storyteller.jpg}}}%
%\par\vspace*{\dimexpr-\baselineskip-\parskip}% Correct for following paragraph
%
%% \parshape <num lines> <indent> <width> ...
%\parshape 11 0pt 0.7\textwidth
%0pt \dimexpr0.5\textwidth+7\baselineskip\relax
%0pt \dimexpr0.5\textwidth+6\baselineskip\relax
%0pt \dimexpr0.5\textwidth+6\baselineskip\relax
%0pt \dimexpr0.5\textwidth+6\baselineskip\relax
%0pt \dimexpr0.5\textwidth+6\baselineskip\relax
%0pt \dimexpr0.5\textwidth+6\baselineskip\relax
...
%0pt \dimexpr0.5\textwidth+3.0\baselineskip\relax
%0pt \dimexpr0.5\textwidth+2.5\baselineskip\relax
...
These are the \glspl{trait} -- the basic character-describing numbers -- which form the core of a \gls{character}. Almost every roll involves adding some Attribute to a Skill. We are going to begin by describing the \glspl{trait} and making a quick \gls{character}.

\section{Attributes}
Every creature in the world has \gls{attribute} to define its basic aptitudes. How fast can they run? How aware of their environment are they? Each Attribute is rated from -3 to +3, where 0 is a sort of `normal' point. The Attribute's number is referred to as its `bonus', because this number forms the basic bonus to dice rolls characters make.

\subsection[Body Attributes]{Body Attributes}
\index{Body Attributes}\index{Physical Attributes}These are the Attributes determined wholly by the character's body. Humans and gnolls tend to excel here, where elves and gnomes are smaller, more delicate creatures. Monsters, beasts and stranger creatures are all described with these three Body Attributes.
...
It'll be easier to understand the system if you make a character as you follow along\footnote{For a complete and brief list of how to make a character, see page \pageref{charactercreation}.}. Either print out a character sheet or make some paper notes as we go. We begin by randomly assigning your race. Nobody picks where they're born after all. Much of character creation is concerned with interpreting your character as it forms -- what kind of person is this you are making? What do the Attribute Bonuses say about them? You will later be deciding on what kind of Skills and training will compliment the character, but the basics will all be random. Grab a pair of D6's and compare the result to the following chart.

\label{character_rolls}

\begin{wrapfigure}{R}{.36\textwidth}
	\begin{tcolorbox}[tabularx={cc},arc=1mm,adjusted title=Attributes]
...
Personally, I've just rolled a `7' so I'm making a human; no surprise there, most characters will be human. I'm getting +1 Strength and -1 Wits. You may think that humans have a normal amount of Wits, but that's just because you've been spending time with too many of them. We and the other races generally consider you to be a little slow on the uptake -- not stupid in the long run, but not terribly bright either. That's fine -- I can roleplay a slightly slow person.
...
Next up, time to roll the Attributes. Roll $2D6$ for each of the Bonuses (or negatives) and record the result after taking your racial modifiers into account.
	Determine your race by rolling $2D6$ and comparing your result to the chart.

Next, determine each of your Attributes by rolling $2D6$ and comparing the results to the Attribute bonus chart on the right.  Continue rolling until all 6 Attributes have a value.

Remember to apply your racial bonuses and negatives to your Attribute totals.
...
Each level of a Skill one has allows the character to have up to 2 Specialisations. For example, someone with Survival 1 might know how to track and build temporary shelters but would count as having Survival 0 when marching. A craftsman with Crafts at level 2 would have 4 specialisations.
...
\index{Simple Actions}A basic action is performed by rolling $2D6$ equal or higher than the \gls{tn} for the action. The more difficult the action, the higher the TN. Players add their character's Attribute and Skill to the roll. Attributes and Skills usually go as high as +3, so a +6 bonus is possible. Very poor characters may be rolling with penalties if Attributes are sufficiently low. All actions are assumed to have a TN of 7 unless your \gls{gm} states otherwise. Don't ask -- just roll!
...
Thenton needed information on whether or not that bard had travelled through here, how long ago and whether anyone noticed anything strange about his book, or singing. He had no desire to wait until things calmed down of their own accord - he would need to leave today.
	...
	Thenton's player uses his Intelligence Bonus (0) and Empathy Skill (+1). The \gls{gm} stated no TN so the TN is 7. Thenton's player rolls $2D6+1$ and the dice show a 4 and a 1, for a grand total of 6. The roll is a failure. No information about the bard is forthcoming, but the \gls{gm} still has some response to describe from the villagers.
...
\index{Resisted Actions}  When \glspl{pc} come into disagreements with \glspl{npc}, the \glspl{pc} pick up the dice, and roll.  The \gls{npc}'s Traits add to the TN.

For example, if a player attempts to compete pick someone's pocket, the \gls{npc} never rolls Wits + Vigilance.  Instead, if an \gls{npc} has a Wits + Vigilance total of +4, the TN for the roll is $7 + 4 = 11$.  The player then rolls against that \gls{tn}.
...
Unknown to the characters, the nura returned to attack the village again some hours ago, slaughtered everyone and began to roast them. Nura tend to act quickly. The smell of roasted meat is coming from human flesh roasting over each fire in the village, while little troves of nura each sit around one hearth, hungrily gnawing on undercooked dinners.
		...
		The \gls{gm} thinks about the difficulty. On the one hand, it is dark (which makes hiding easier) and there are some signs of battle in the village, such as blood on the grass. On the other hand, the darkness stops the characters seeing the signs of battle. She decides that the various factors cancel each other out and keeps the base TN of 7. She adds the hobgoblins' score to this - the highest score counts since any of of the nura might spot the characters, but all the hobgoblins have the same score. They have Wits -1 and no Vigilance Skill; the hobgoblins' score is added to the TN for a final TN of 6. Meanwhile, Thenton still has a Wits + Vigilance total of -1.
...
\subsection{Margins}
\index{Margins}Most actions are either a success or a failure, but sometimes the \gls{gm} will request to know a roll's Margin -- i.e. just how well the character has succeeded at the task. The Margin is the number of points over the TN a roll has gathered. If the TN is 9 and a player scores 11 then the Margin is 2.
...
\iftoggle{verbose}{
	...
	\iftoggle{verbose}{\begin{wrapfigure}{h!}{.4\textwidth}
		\includegraphics[width=.4\textwidth]{images/Boris_Pecikozic/nura_jump.jpg}
	\end{wrapfigure}}

		While previously the players rolled to hide against their opponent's ability to spot them, this time they roll to see if they spot a hidden opponent. The character with the highest score is Arneson, with Wits 0 and Vigilance +1. The \gls{gm} decides that the hobgoblin should use his Speed Bonus of +1, and his Stealth Skill adds +1 again. The nura's score of +2 adds to the basic TN of 7, producing a final TN of 9. Arneson's player rolls $2D6$ to produce a final score of `12' - the roll is a success and the margin is 3. Since the margin is quite good, the \gls{gm} decides that the troop leave the area before they are engaged and gain a +3 bonus to running away.
...
\includegraphics[width=.5\textwidth]{images/Roch_Hercka/xp-1.jpg}
\end{wrapfigure}
}{}
...
At the end of each combat encounter, the total \gls{xp} for all monsters is counted up and 1\gls{xp} per player is taken off the total. As a result, a group of five \glspl{pc} killing two gnolls at a time will gain far less \gls{xp} than taking on ten gnolls in a single encounter. The remainder is then divided equally among the players, with spare \gls{xp} waiting in a `pot' for later. Fleeing monsters are worth half the usual \gls{xp}.
...
\subsubsection{The Discount}
\iftoggle{verbose}{Standing alone against a towering ogre is a nightmare, but three warriors standing against three ogres can be much easier.  A battle against thirty goblins can really take its toll, but three different battles against ten goblins can be child's play.  To represent this, we have \textit{the \gls{xp} Discount} -- a price you pay for every member of the party.\par}{}

For every member of the party, that many points are deducted from one monster's \gls{xp} value (to a minimum of 0).  If the party has two members, the first two monsters have 2\gls{xp} deducted from their total value.  If the party has five members, the first five monsters have 5\gls{xp} deducted from their total.

If a single warrior fights a dragon worth 22\gls{xp}, then the warrior receives 21 \gls{xp}, because 1\gls{xp} is always removed from the total.  If he fights 10 zombies worth 2\gls{xp} each, then he receives ($10 \times 2 - 1 =$) 19\gls{xp}.

However, if five characters are fighting the 10 zombies together, they each deduct 5\gls{xp} from a single monster.  The first five zombies are worth nothing, because each net ($2 - 5 = $) 0\gls{xp}.  Only the last 5 zombies count, bringing 10\gls{xp} in total.

\subsection{Spending \gls{xp}}
\iftoggle{verbose}{
	\begin{wrapfigure}{O}{.5\textwidth}
\includegraphics[width=.5\textwidth]{images/Roch_Hercka/xp-2.jpg}
\end{wrapfigure}
}{}
...
\begin{tcolorbox}[arc=1mm,tabularx={p{.3\textwidth}XX}]
	...
	\textbf{Armour} & & \\\hline \index{Armour!Cost}
	...
	Camping equipment &  2-4 &  {1 sp} \\\index{Camping}

	One meal &   &  2 cp \\

	Room for the night &   &  {1 sp} \\\hline
...
\newcommand{\initiativechart}{
	...
	\begin{tcolorbox}[title={Initiative Chart},arc=1mm,tabularx={|p{.5\textwidth}c}]

Action & Initiative Cost\ \  \\\hline
...
\begin{wraptable}[21]{r}{.4\textwidth}
...
Quick Actions can interrupt the usual Initiative priorities. Any time someone attempts a Quick Action, just so long as they have some Initiative score left, can take their action immediately. If two characters interrupt the Initiative flow with Quick Actions then whoever currently has the highest Initiative Score goes first.
Quick Actions allow characters to guard someone as soon as they see an attack impending upon a friend, or to run away before someone approaches them. The quick actions are shown above by an asterisk.


\subsection{Evasion}\index{Combat!Evasion}

The basic TN to hit a character is 7; characters' Evasion score adds to this. The basic Evasion score is given by a character's Dexterity Bonus. For example, a character with a Dexterity bonus of +2 would have a TN of 9 to hit.
...
\includegraphics[width=.6\textwidth]{images/Roch_Hercka/stances.jpg}
\end{wrapfigure}
}{}
...
\subsection{Stacking Damage}

\index{Combat!Stacking Damage}Damage Bonuses cannot extend forever. If the Damage bonus ever exceeds +3 then 4 points of the bonus are replaced with a die. Therefore, what might usually be $1D6+4$ Damage becomes $2D6$ Damage.
...
\index{Combat!Perfect Strikes}Rolling a \gls{natural} `12' in combat, i.e. rolling two 6's, means the roll was a Perfect Strike. A Perfect Strike is guaranteed to hit even if it doesn't reach the opponent's TN, it ignores both Partial and Complete armour (covered below) and it grants +2 Damage.
...
\subsection{Death}
\index{Death}Once a \gls{pc} reaches 0 HP they must make a \index{Vitality Check}Vitality Check in order to stay alive. This is rolled at TN 4 plus one for every negative HP level. For example, if someone with 3 HP left were to take a further 6 Damage, this would put their at -3 HP. That makes the TN 7 for the Vitality Check.
...
\glspl{npc} roll Vitality checks at a basic TN of 7 instead of 4.\iftoggle{verbose}{\footnote{Traits such as Strength do not affect the Vitality check because in a way, they already have.  Stronger characters already have more Hitpoints, which has already kept them farther from death.}}{}
...
\section{Fate Points}
\label{fate_points}\index{Fate Points}At this point you might be wondering how anyone is going to survive past their first battle. 6 or 7 HP is not a lot when the Damage is often $2D6$ or higher. The mechanism which saves the plot-important character is \gls{fp}. Every time someone would lose HP, the character marks off \gls{fp} instead and it is stipulated that the attack in fact misses, because the gods have fated this person to live another day.
...
\newcommand{\weaponschart}{\begin{tcolorbox}[arc=1mm,tabularx={p{.15\textwidth}rrccl}]
	\textbf{Light Weapons} & \textbf{Dam.} & \textbf{Init.} & \textbf{Ev.} & \textbf{Wt.R} & \textbf{Knacks} \\\hline

	Cudgel & +2 & 0 & 0 & -3 & Stunning Strike \\
...
\begin{wraptable}[8]{r}{.35\textwidth}
...
\iftoggle{verbose}{\begin{wrapfigure}{R}{.4\textwidth}
\includegraphics[width=.4\textwidth]{images/Roch_Hercka/vitals_shot.jpg}
\end{wrapfigure}
}{}

When attacking an opponent in armour, it is possible to make a shot so precise as to get a gap in a helmet, strike an opponent in the eye or slide a blade between overlapping plates. To get a Vitals Shot, one simply needs to roll high enough over the creature's regular TN and all armour (meaning \gls{dr}) can be ignored.

For partial armour, anyone rolling a Margin of 3 (i.e. 3 points above the TN) ignores the \gls{dr} from the armour. If the regular TN is 8 then any roll of 11 or greater counts as a Vitals Shot. Complete armour requires a Margin of 5 to ignore the armour, so if the TN were 10 then a Strike would require a total of 15 to bypass the armour. Perfect armour cannot be bypassed by a sufficiently high roll.
...
\subsection{Weight}
All armour's has a Weight Rating, just like any other item. The Weight Ratings above are for Partial Armour. If anyone wears Complete armour the Weight is increased by 1, so Complete chain armour which comes past the knees, has a helmet and uses arm-guards, would have a Weight Rating of 2.
...
Well, obviously they attack characters, but the \gls{gm} does not have to roll for their attack.  Instead, players roll to \textit{avoid} incoming attacks at TN 8 plus the monster's Strike Factor.

If a player faces down a hobgoblin with a Strike Factor of +2, then the player must roll with their Evasion Factor at a TN of 10.  If the player fails the roll, the monster hits.

Instead of \glspl{npc} making a Vitals Shot when they roll 3 above the TN to hit a PC, the PCs receive a Vitals shot when they fail an Evasion roll by 4.  If the TN to Evade an enemy is 10 then rolling a 9 means the player has been hit, but rolling a 6 means the PC has been hit by a Vitals Shot.\footnote{If this looks at all odd, remember that when players attack at a TN of 7, they make a regular hit on 7, 8 or 9 and a Vitals Shot on 10 or more.  When they Evade at TN 8, they get hit on 7, 6 or 5, but receive a Vitals Shot when they roll a 4.}  Players wearing Complete Armour receive a Vitals Shot when failing an Evasion roll by 6 points or more.
...
\includegraphics[width=.4\textwidth]{images/Boris_Pecikozic/nura_brawl.jpg}
\end{wrapfigure}
...
Arneson's player needs an 8 to hit the ogre, so he rolls and adds +1 for his combat bonus. His total is `11' and he hits. He has already rolled his damage die at the same time. It landed on a `2'. He adds +2 for his Strength and +1 for the sword's Damage bonus for a total of 5. The ogre shrieks in pain as Arneson's sword sticks in. Arneson's action took 6 Initiative points so he goes down to `2'. The second the action is resolved the and the \gls{gm} moves on.

``Seven!'', she cries. Thenton's player jumps in. He only deals $1D6+2$ Damage but he has a much better Combat Skill of +2. He rolls for the strike but misses the ogre with a `5'. His Initiative score reduces to 1.
...
Two hobgoblins hit Arneson. The first one rolls a 10, which is just enough to hit him. The hobgoblins' Strength is +2 and the battle axe adds +3 Damage for a total of +5. 4 of the Damage is replaced with a die, so the hobgoblin is rolling $2D6+1$ Damage. The total is 6. Arneson's player first reduces that by his chainmail's \gls{dr} of 4, leaving 2 Damage. Instead of taking that Damage he marks off 2 Fate points and declares that the attack in fact misses. The next hobgoblin hits with a score of 13; it only needed 10 so that's a margin of 3. Since Arneson is only wearing Partial chainmail, attacks with a Margin of 3 bypass his armour. The axe is coming down towards his unarmoured shin-bone and the Damage rolled is 9. He marks off his last 9 FP rather than taking any Damage. Any further Damage is coming straight off his Hit Points.

Hugi finally releases his crossbow, but in all the confusion misfires. He spends 4 Initiative to twang a bolt uselessly past the ogre's head. He is acting again at Initiative 1.

``Four!'', shouts the \gls{gm}, then she smiles. ``The ogre pushes forward with its club then reaches out to grab Hugi. I roll ... nine''

Hugi's player isn't happy, as that is enough to hit him, or in this case grab him. In fact with his crossbow out rather than a defensive weapon, people only need a 7 to hit.
...
The other players want to attack, but the ogre is making a movement action - these count as Quick Actions so they are allowed to operate outside of the usual Initiative. Hugi disappears behind the crowd. The ogre's Initiative reduces to 0 and he is done for the \gls{round}, but Arneson and Thenton each have one action left.}}{}

\section{Complications, Options \& Manoeuvres}
\subsection{Blind Rage}\label{blind}
\index{Blind Rage}Weapons can grant a bonus to the wielder's Evasion Factor because the wielder is keeping people at bay with it -- a spear might be waved in an opponent's face in a threatening manner or a sword might be on the ready to attack if someone gets within its range. However, this marvellous defence only works against people who care about being hit. Anyone can choose to attack someone while ignoring their opponent's weapon's bonus to Evasion; the penalty is simply that the opponent can choose to make a single Sneak Attack immediately. This attack is made outside of the normal Initiative order and gains +4 to the attack and +2 to Damage.\footnote{See page \pageref{sneakattack}.}
...
\subsection{Brawling}
\index{Combat!Brawling}\index{Brawling}Punches and kicks all use the Combat bonus. Such attacks inflict Fatigue Damage. Everyone gains a \gls{dr} against Brawling Damage equal to their Strength Bonus, which stacks with armour (\gls{dr} cannot be negative). This counts as Complete armour, so hitting someone in Partial chainmail with a TN of 8 and a Strength of +1 would mean they have a total \gls{dr} of 6. However, an attack score of 11 would mean that the Partial armour's \gls{dr} could be ignored, leaving only a \gls{dr} of 1. An attack score of 13 would ignore both types of \gls{dr}, leaving nothing at all. Attacks which bypass a body's natural armour count as normal Damage as such attacks might hit vulnerable locations such as the eyes or crotch or twist an opponent's arm till breaking point.

\subsection{Drawing Weapons}
\index{Combat!Drawing Weapons}Drawing a weapon costs 2 Initiative if it is placed in an easy place to draw, like a scabbard no the side of a belt. If a character holds weapons on the back or in a bag, it costs 8 Initiative to remove them. If a knife's stuffed inside a pack, the \gls{gm} may stipulate a number of \glspl{round} required to draw the weapon.

\subsection{Flanking}
\index{Combat!Flanking}\index{Flanking}Someone can flank and pester an enemy in combat, always remaining at their anterior side, in order to expose their to further attack. The target's Evasion Factor is reduced by 2 plus the attacker's Combat Skill (so someone with a Combat Skill at +2 could reduce an enemy's TN by 4). Rather than taking up a finite amount of Initiative, the flanker simply persists for as long as they please -- and for that time the target receives the penalty. A second flanker can add a -2 penalty and any more add -1, up to a maximum total of a -6 penalty.

The target can ignore being flanked if they choose but the attacker has the option of immediately making an attack which counts as a Sneak Attack (see below, page \pageref{sneakattack}).

\subsection{Grappling}
\index{Combat!Grappling}Grappling a character first requires a grab attack. This functions as a normal attack but causes no Damage. If it hits, the attacker has successfully started a grapple. During the grapple, one grapple \ attack is made any time it is possible -- no other actions may be performed by the grappler (and probably not by the grappled person).

During the grappling rolls, the two make an opposed roll of double their Strength Bonus, plus their Combat Skill. The TN to maintain the grapple is 6, at which point Fatigue Points are applied just as with a Brawling roll. If the Margin is 3 or more then normal Damage is applied instead of Fatigue. If targets are wrestling while using a dagger then this Dagger deals Damage as per usual on any successful roll, no matter what the Margin.

If any grappling roll misses, the target has broken free of the grapple. Until that point, if anyone attacks a grappled or grappling character, they can make a Sneak Attack. Those who Initiate the grapple can, of course, simply get up and move rather than waiting to be attacked when they are on the ground.

Characters who are being grappled can stop struggling and simply allow themselves to be attacked. Their opponent can make a Sneak Attack, but this allows them to act in any way which does not require the use of one's hands, such as casting certain spells.

\subsection{Guarding}
\index{Guarding}It is possible to stick by someone in combat and guard their from attack, at least from one side (but not from everywhere at once). Anyone making a Combat attack against that person must go through you first -- they roll to hit you and, if successful, can chose to Damage you or make another roll with a cost of 0 Initiative to hit the target. If someone attempts a missile attack, you can fend that off as well, but only if you are Keeping Edgy (see below) for missile attacks. Guarding someone takes 2 Initiative and counts as a Quick Action.

\subsection{Half Swording}
\index{Combat!Half Swording}It is possible to hold a sword by the blade and use the guard to bludgeon one's opponent. This manoeuvre allows the weapon's Speed Bonus to be added to its Damage instead. It takes 2 Initiative points to change how one holds the sword.

\subsection{Holding Off}
\index{Combat!Holding Off}Anyone can wait to see what the battle brings -- the character simply lowers their Initiative and can jump in at any point, acting at one Initiative higher than a declared action. This can be used to defend people by moving in front of them, or to identify and then attack a spellcaster by waiting until someone casts a spell.
...
\subsection{Keeping Edgy}\label{edgy}
\index{Keeping Edgy}The character can take a moment to note their \index{Dodge!Long-range}long-range surroundings, including archers and potential spell casters. This takes only 2 Initiative points and for the rest of the \gls{round}, any time the character is being fired upon in combat they can use their basic Speed Bonus in a resisted action to leap out of the way of an incoming missile or targeted spell, such as a fireball. Spells which simply target people by gaze or magical effects such as polymorphing are unaffected.

\subsection{Passing Attacks}
\index{Combat!Passing Attacks}When someone is attempting to run past others, they have some opportunity to hit when the character is passing. This can be any normal attack, but counts as a Quick Action, jumping ahead in the Initiative Score. The attack costs the normal amount of Initiative.

\subsection{Ram}
\index{Combat!Ram}In combat, it is possible to scare, push and stab at someone to force them to move backwards. The attacker spends 3 Initiative points. The defender can either attempt to resist, or can simply acquiesce and move back. When moving back, targets are pushed back 1 square; the attacker's Strength adds to this and the opponent's Strength decreases it. Characters can sacrifice the use of 1 point of Strength to push back an additional person.
...
\subsection{Surprise}
\index{Surprise Attacks}\index{Combat!Surprise}When sneaking up on someone, the instigator uses their Dexterity + Sneak while the target uses Wits + Vigilance. If the usual TN of 7 is met then the attacker gets one Sneak Attack before switching to normal combat. Surprise Attacks always receive a penalty equal to the weapon's Weight Rating, if it is above 0.

\subsection[Spell Casting in Combat]{Spell Casting in Combat}
\index{Combat!Spell Casting}Spell casters are assumed to be focussing on their spells and using both hands for that purpose rather than weapons. They use their Wits for the Initiative bonus rather than Speed and receive no Combat Skill Bonus to Evasion -- they only use their basic Dexterity score.
...
\index{Combat!Two Weapons}A character using two weapons -- perhaps a shield in one hand and a sword in the other -- can use the best Speed, Strength and Dexterity bonus from all weapons. Each weapon will have to be held in one hand, increasing its Weight Rating by 2.
...
\subsection{Sneak Attacks}\label{sneakattack}
\index{Combat!Sneak Attack}\index{Sneak Attack}When attacking while unseen, the attacker gains a +4 bonus to the attack and a +2 bonus to Damage. Opponents cannot use any Evasion bonuses from Dexterity, weapon Bonuses or the Combat Skill.
...
\item The leader of each group rolls Initiative and each character adds their Initiative Factor.
...
\index{Ranged Combat}Projectiles have their own Skill which is bought just like the Combat Skill and always add the Dexterity Bonus to make an attack. Archers roll to hit and then Damage, just as with Combat. The TN is always 6 plus one for every five full squares away the target is. Targets 14 squares away would have a TN of 8 to hit. Most targets cannot use any weapons to add to their Evasion Factor (except shields) but can use the Speed Bonus to evade missile attacks if they are on the run or Keeping Edgy.
...
Weapons which were never made to be thrown, such as swords or axes, receive a -2 penalty to hit, i.e. they are thrown at a basic TN of 8 rather than the usual 6 for projectiles.
...
\includegraphics[width=.33\textwidth]{images/init_hobgoblins.jpg}
\end{wrapfigure}
The players wonder if they want to add their Dexterity Bonus and Weapon's Evasion Bonus to Strike opponents instead of Evading attacks, but at this stage defence is more important, especially for Hugi - currently being pinned down by an ogre, and Arneson, currently down to only his Hit Points.
		...
		The characters roll to get their bearings and move to act. They achieve only a `4', so Thenton will act at Initiative 7. The \gls{gm} rolls for the nura and achieves `11' - with Speed -1 he will act on Initiative 10.

		``Twelve!'', the \gls{gm} rolls a Morale Check for that last hobgoblin. It is wounded and outnumbered. The TN is 12 and it can add its Combat bonus, but the roll still fails.

``The last hobgoblin backs up. Eleven! It flees down the mountain towards its allies, many of whom are still rolling down the hill.''

		``Ten!'', goes the \gls{gm} to start things off, and immediately rolls for the ogre as it tries to eat Hugi's face off. This will count as a grappling roll, so he and Hugi will use double their Strength Bonus added to their Combat Skill. Unfortunately Hugi has neither Strength Bonus nor Combat Skill, so the ogre gets a straight +12 bonus; the roll succeeds before it is even made, and succeeds by a margin of 3: that means Damage is inflicted. The ogre only adds Strength - of course his massive club is useless for the attack. His Strength of +5 means he will roll $2D6+1$ Damage for a total of 4. Hugi is safe for now as the ogre luckily bites down on dwarvish helmet as Hugi's player marks off 4 FP.
...
Hugi's player marks off his last 3 FP then 2 HP, noting that he could have been dead there.
...
``Well'', she ponders, ``you certainly don't outnumber them. They are not half dead. They might make a moral check at TN 4 but they would get a +1 bonus for their Combat Skill. So no, not really. They are not terrified of Thenton waving his sword about, even if he took acting classes''.
...
The TN is 6 plus the enemy's Speed + Athletics Bonuses. Failure means the characters are instantly caught, before they are able to run anywhere. If the players hit the TN they manage to run through 1 area while being chased. For every Marginal point, they run through an additional area. If the Margin is ever 3 or more then they completely evade the enemy. If the party obtain less than total success, they and their pursuers both move and must roll again.

The table is a guide to an unaltered roll. In most situations enemy Traits will affect the actual results of such a total by increasing or decreasing the TN.
...
\begin{exampletext}
	...
	The basic TN for such actions is 6 and the \gls{gm} lowers it by 2 because the trio have a good headstart. The hobgoblins add their Speed Bonus of +1 for a final TN of 5. The party roll an 8 but unfortunately Hugi isn't the fastest of people - he's only four feet tall after all - so his score is 7. They needed that 8 to completely get away. Arneson and Thenton decide they're going to keep pace with him rather than running ahead. They're not caught yet, but run through three different distinct areas before making another roll.
...
\index{Falling}Characters who fall from a height suffer 2 Damage per square the character fell. 2 Damage alone converts to $1D6-2$ Damage, while 4 Damage would simply be $1D6$ and so on. Characters falling straight downward can attempt to mitigate 4 Damage by rolling Dexterity + Athletics at TN 9. Those falling forward and down in an arc can try to roll along the ground to mitigate the Damage; they roll Dexterity + Athletics at TN 7 and a successful roll indicates that they reduce incoming Damage by 4.
...
\subsection{Casting}
Spells are cast by spending a number of \gls{mp} equal to the spell's level, so 1st level spells always cost 1 \gls{mp} and 3rd level spells always cost 3 \gls{mp}.  The character then makes a roll against some TN, and the spell is cast if successful.
...
If a caster has no \gls{mp} left, they can still cast spells by paying the cost with HP instead of \gls{mp}. The magical energies pull the power they need from the blood and bones of the caster, leaving them with a bleeding nose, raging headache and sometimes stranger effects such as acidic pustules or discoloured skin patches. Many a desperate caster has died through the use of their own magic rather than an enemy's sword; a wizard with their back to the wall is a dangerous opponent indeed.
...
For example, Tauron the elven sorcerer casts a spell on himself to appear as a gnome -- all the better to blend into surrounding society. He spends 1 \gls{mp}. Later, he enchants an animal to be his companion for 3 \gls{mp}. Normally, his maximum \gls{mp} is 6, but he is currently reduced to a maximum of 2 \gls{mp} so long as he continues to be a bear-riding gnome.

These still-active spells are known as \index{Magic!Standing Spells}Standing Spells. Some mages operate by continuously casting different spells and then going `empty' when the mana is gone. Others typically operate with Standing Spells alone, casting everything they might need before the day begins and leaving their useful spells `running' but leaving themselves unable to cast more.
...
Mages who take their time over spells can attempt a Ritual Spell -- they cast it as a Resting Action.\footnote{See page \pageref{restingactions}.} The mage can gain mana slowly, spending some, drawing more from a mana stone or item, then spending more before finally casting the spell. The mage can gather a number of \gls{mp} equal to double their normal maximum \gls{mp}, ignoring Standing Spells. Ritual spells can also be cast as a team effort -- any number of spell casters who are on the same \gls{path} of magic can cast any spell they all know together. They can each invest \gls{mp} to create Standing Spells, and thereafter any one of them can cancel the spell.

\subsection{Subtle Spells}
Casting an illusion or enchantment on someone with a flashing, loud and generally obvious spell can be quite a give away. Any caster can attempt to cast a spell while simply whispering and moving their hands slowly and subtly.  The TN is raised by +2 as this is a very difficult way to cast anything.
...
The TN for this spell is 7 plus the target beast's Wits + Aggression Skill (the Skill which replaces Combat for beasts). The caster rolls their Intelligence + Beast Ken. For example, a creature with Wits +1 and Aggression +2 would be at TN 10 to affect.
...
Forest's Friendship works on all creatures without an Intelligence score. Umber hulks, bears, birds, et c. - all can be affected with the language of the forest. However, mammals are the easiest to work with. The DM should add to the TN to affect birds, insects and other non-mammalian creatures.
...
The mage casts a dim light, light the light of a torch, which surrounds any target.
...
\spell{Sunray}{Continuous}{Survival}
...
The mage can also send out whispering messages on the wind, making their words carry across mountains and past houses. The winds can carry up to ten words, and the message can wander for miles around -- up to 2 areas plus 1 per Intelligence Bonus the mage has. This requires an Intelligence roll at TN 7 with a -1 penalty for every area miles away the target is. The target must be easily identifiable, as the wind acts as a semi-sentient extension of the mage's mind, watching out for the target with its own narrow version of vision.

If the caster has access to Metamagic's first level then extending the range means the spell can travel any distance, although longer distances can substantially raise the TN and make it difficult for the spell to identify its recipient. Extremely long-range spells can also take a long time to reach the intended recipient, forcing the mage to cast Whispering Wind as a standing spell and leave it active until they guess the correct amount of time until the spell will find its target.
...
The system is the same as the first level -- the caster rolls their Intelligence + Beast Ken Skill against a TN of 7 plus the target's Wits + Aggression. The TN is raised for strange creatures such as insects or birds.

After a successful spell ends, another roll is made at an identical TN. Success indicates that the creature has a positive outlook towards the caster and that they have gained enough rudimentary intelligence to follow basic commands and may follow the caster for as long as they wish as a companion.
...
This spell has a basic TN of 7, adjusted for more sparse fauna. Once the spell ends, the plant returns to its original form, slowly retreating, shrinking or just dissolving like fading autumnal leaves.
...
The poison inflicted by this spell heals at a rate of 1 per scene, though a successful Intelligence + Medicine task can increase the healing rate at the discretion of the DM. The poison is not removed from a target after the spell has ended.

Those caught in the area of effect when the spell takes place can resist it with their Evasion Factor while the caster uses their Intelligence + Survival. For example, if the caster had a +4 Bonus from Intelligence + Survival and targeted someone with a current Evasion Factor of +2, they would have to roll against a TN of 7 to activate the spell, but would have to reach a TN of 9 to affect the target before they jumped away from the venomous plants. Alternatively, PCs attempting to evade the growth would have to roll against TN 11 to jump out of the way, because the basic TN of 7 would add the caster's Bonus of +4.
...
The DM may adjust the TN of the spell, depending upon the quality of the basic plant matter which the caster is working with and encouraging growth from. Working with a wild swamp, teeming with tree-roots and floating lillies might lower the TN, while casting a spell upon a sparse rock garden should increase the TN.
...
The caster can affect a total area equal to \arabic{spelllevel} squares plus their Wits Bonus. The basic TN is 7 which increases for young, living wood and decreases for older, already-degraded wood. Other natural substances, such as leather or cloth can also be made to degrade, but only once substance per casting can be affected. The caster uses their Intelligence + Survival to roll, and any success instantly destroys all targeted matter.
...
The final level of aldaron allows a mage to summon a mighty storm, possibly including rain, lightning, thunder and hail, as the caster desires. The exact effects of the storm are in the hands of the DM but potential effects include knocking down small buildings, inflicting Fatigue Points on anyone in the storm and random blasts of lightning. A higher Margin suggests more powerful effects. The spell extends from the caster in all directions by a number of areas equal to five plus their Wits.
...
Conjuration spells targeting larger items are always more difficult.  The TN for anything besides a gas, such as air, is always increased by the Weight Rating.  In the case of living targets, the Weight Rating is always equal to their \gls{hp}, so targeting someone with 6 \gls{hp} would increase the TN by 6.

Any character can decide that a conjuration spell targeting them by spending 5 FP.

%%%%% Alternate form:
% +1 level for changing forms - solid/ liquid/ gas
% +1 level for area
% +1 level for complicated items, consisting of multiple components, or a complex shape.
...
The caster changes any solid target to stone.  The TN is 7 plus the target's Hit Points.  Once the spell is over, the target turns back to normal.

Metal cannot be targeted by this spell.

\spell{Slime}{Continuous}{Survival}

The caster turns any nearby liquid into a slippery slime.  Anyone running across the area makes a Dexterity + Athletics roll, TN 7 + the caster's Intelligence.  Some kind of liquid must be in the right place for the spell to work.

Casters acting quickly often carry their own water.  Throwing water requires 8 initiative for using an item, as usual.

\spell{Web}{Continuous}{Crafts}

The caster turns any liquid inta a viscious, sticky substance.  Anyone coming into the liquid gets stuck, and much take a full movement action trying to get free.  This requires a Strength + Athletics roll, TN 7 + the caster's Intelligence.

Webbing cannot be used instead of rope -- it's too elastic, and tends to snap when stretched.

\spelllevel

The caster can now change from any form to any other -- air can become rock, iron can become water, and water can turn to ice or stone.

The only limitation is that precious metals, such as copper, silver and gold, cannot be created or affected.

\spell{Artefact}{Continuous}{Varies}

The caster can now transform things into complex items.  Air can become a complex, and rich scent.  Solid wood can turn into a sword, or rope.  Water can become beer, wine or even acid.

\spell{Acid}{Continuous}{Survival}
The caster can turn any liquid into a potent acid.  Acidic substances deal $1D6$ Damage plus the caster's Intelligence bonus.

\spell{Desintegration}{Continuous}{Survival}

The caster can turn any solid item, animal or person into air or water.  Once the spell wears off, the target does not reform and therefore dies instantly.  The TN is 7 plus the target's Weight Rating.

The spell can target weapons, such as spears, but never metal weapons such as swords -- the wooden handle's generally not visible enough to be targeted while in use, and metal cannot be transformed at this level.

\spell{Improved Slime and Web}{Continuous}{Survival}

Previous spells -- slime and webbing -- can be cast by transforming the air into the desired substance.  The webbing or slime can occupy up to \arabic{spelllevel} squares plus the caster's Wits Bonus.

\spell{Wall}{Continuous}{Crafts}

The caster can turn water into ice, or air into stone, or otherwise make any barrier.  The barrir takes up up to two squares plus the caster's Wits Bonus.  Such barriers can only be broken with a Strength + Crafts roll, TN \setcounter{enc}{7}\addtocounter{enc}{\value{spelllevel}} \arabic{enc} plus the caster's Intelligence Bonus.

\spell{Prison}{Continuous}{Crafts}

The caster freezes the water around the target, or turns surrounding air to stone, imprisoning them.  While the spell is being cast, the target can attempt to break free as a Quick Action, costing 2 Initiative, by rolling Strength + Athletics.  The TN is 7 plus caster's Intelligence Bonus plus Crafts.

If the spell completes, the TN to break free increases by 2.

\spelllevel

At this level, the prohibition on metal is lifted, with the one exception of gold.  Mages can also summon simple substances over a wide area.

\spell{Greater Artefact}{Continuous}{Crafts}

The caster can turn any substance into a similarly large item of any complexity.  Air is common, so casters usually target air, resulting in the belief that conjurers can pull carts or weaponry from nothing.

Air is also popular as it's not difficult to manipulate.  As usual, any other target increases the basic difficulty by an amount equal to its Weight Rating.

The caster can create any item, occupying a maximum of \arabic{spelllevel} squares plus the caster's Wits.  Cards, cabins, carts, or even siege weapons can be summoned.

If the \gls{miracleworker} can cast the spell quickly, rocks can be conjured above a target's heady, dealing $1D6 + \arabic{spelllevel}$ plus the caster's Intelligence Bonus in Damage.  However, in such cases targets who are Keeping Edgy can resist with their normal Evasion score, which increases the TN.

\spell{Disarm}{Continuous}{Crafts}

The caster turns a sword, dagger, or any other weapon to stone, wind, or anything else.  It instantly becomes useless.

\spell{Teleport}{Instant}{Academics}

The mage teleports the target a short distance -- up to \arabic{spelllevel} squares plus the caster's Wits.  As usual, the target can cancel the spell by spending 5 \gls{fp}.

\spelllevel


\spell{Portal}{Instant}{Survival}

The \gls{miracleworker} can open a magical portal from one place to another.  The only restriction is that the two areas can be at maximum \arabic{spelllevel} + the caster's Wits apart.

Portals cast underwater send water gushing to the other side.  Portals can also be cast with one on the ground, and another on a ceiling, so that anyone stepping inside falls through.

\spell{Gold}{Continuous}{Crafts}

Solid items can finally be turned into gold.  Alchemists are always suspected of trickery when paying for anything.

\spell{Summon Item}{Continuous}{Crafts}

The caster can now summon complex items, made of multiple parts.  Carts, beer, weapons, almost anything.

\spelllevel

\spell{Gate}{Continuous}{Ether Lore}

The mage can finally open a portal from any known place to any other known place, without restriction.  The mage needs only a passing familiarity with the location, and even a detailed third-hand account suffices.

\sphere{Enchantment}

\iftoggle{verbose}{
	\begin{wrapfigure}{T}{.3\textwidth}
\includegraphics[width=.3\textwidth]{images/Roch_Hercka/elvish_enchanter.jpg}%
\end{wrapfigure}
}{}
...
The spell attracts undead to the item, who feed on the destruction of life.  Any undead in the area will follow the target, just as if it were a person.  With mindless undead, this works without failure, though intelligent undead can plainly understand that the item is not a person.

\spell{Mind's Healing}{Continuous}{Empathy}

The enchanter can also cut any ill effects caused by level 1, 2 or 3 Enchantment spells. If used as a standing spell, the target becomes immune to such spells unless they want to be affected by them.

\spell{Fear}{Continuous}{Deceit}
\Glspl{npc} his by this spell suffer a -2 Morale penalty.  \Glspl{pc} his by this spell are not allowed to know their current \gls{fp} total -- the \gls{gm} tracks it instead.
...
If the enchanter does not have any languages in common with the target then the TN is 9 rather than 7. The target cannot send messages back.
...
If an \gls{npc} enchanter intends to cast this on a PC during a scene, the DM is encouraged to simply make the resisted roll for the spell. If the player fails the roll then the DM can infer what probably would have happened had the scene played out and skip to the next scene, telling the player that something important might have happened, but that they cannot remember any of it.
...
If the target would normally take a Morale check, the caster's Intelligence + Deceit are added to the TN for as long as the spell is in effect. Targets completely unable to flee will continue to fight.
...
\spell{Auguary}{Continuous \& Instant}{Academics}
...
Once cast, the \gls{gm} looks through the upcoming encounters and picks a description.  It could be random boxtext, a note about an upcoming character, or a metaphor involving upcoming events.
...
The priest blesses people with the favour of the gods, pulling them from harm's way. The target `heals' or regenerates $1D6$ \gls{fp} plus the priest's Intelligence Bonus. This cannot take the target above their maximum \gls{fp} score.
...
The priest spends 1 Story Point and gains an addtional 5 Story Points plus their Intelligence Bonus, which must be spent immediately.  This can be used on a summoning miraculous help, such as a crew of soldiers who have a debt to the priest, or a magical ally.

As usual \gls{gm} is free to veto any ideas, but the player is also allowed 

\spell{Resurrection}{Instant}{None}

The priest summons the soul of a recently deceased person back to their body. If they are beyond -3 Hit Points, they must roll a \index{Vitality}Vitality Check again to stay alive, but this time with a +5 bonus. There is no roll for the caster -- the spell is automatic and the spell is instant, so the effects need not be maintained. If the spell is made into a standing spell then the effects count as being continuously cast.

The spell also heals the target of a number of HP equal to half the Margin. This cannot bring the target above 0 HP. For example, if a PC were at -7 HP they would normally make a Vitality Check at TN 11. Adding in the Bonus would make the adjusted TN 6. If the Vitality Check were a roll of 11 then the Margin would be 5 and the character would heal 3 HP, going up to -4 HP. This healing should be understood as a retroactive blessing from the gods, indicating that the Damage sustained was not nearly so bad as was once thought.
...
\sphere{Force}
\index{Force}The mage can shape pure energy, pushing and pulling at the world with the power of their will alone. They can form shields, pick up weapons and grind targets into the ground as if with an invisible, giant, floating hand.
...
The mage rolls Intelligence and Vigilance at TN 6 plus the spell's level. The spell covers a progressively larger area depending upon the level used. No roll is required -- the spell is automatic and lasts until dispelled.
...
\begin{tcolorbox}[arc=1mm,tabularx={ll}]
	...
	2 & 10 squares plus the caster's Force sphere rating. \\
...
The mage can erect a magical force field, similar to mage armour, over a doorway to make it more difficult to break through. The TN to break through the door increases by an amount equal to double the level of the force sphere being employed plus the mage's Intelligence Bonus. For example, if a door were at TN 8 to burst through, a mage with Intelligence +2 could cast the second level of the force sphere, raising the TN to 14.
...
\sphere{Illusion}
\index{Illusion}Illusions create a facsimile of sounds and sights out of pure magic. The thing created might look like a hat, a coin, a rat or even a dragon at higher levels. Illusions also create convincing sound -- loud echoes, the sound of nearby battle, perhaps even imitating an enemy commander's orders in battle. However, illusions are little more than coloured air and noise -- once touched they fade away. They are frightening and if properly used can defeat armies, but are not perfect weapons by any means.

Seeing through an illusion is always an opposed roll -- the illusionist adds their Intelligence Bonus while anyone seeing or hearing the illusion resists with Wits + Vigilance. The DM should roll in secret if an illusion is present, and inform the players of the deceit if the players spot it. If someone has a reason to suspect that something is an illusion, they should receive a +2 bonus to resist it. The party also receive a bonus for multiple people who might spot the illusion, as per the standard Vigilance Skill rules.
...
For example, a gnome creates an illusion of a fleeing gnoll with a great bundle of treasure in his hand, hoping the PCs will chase after him immediately. His Intelligence is +2 and his Academics is at +1 though he has no appropriate specialisations, so the players are rolling at TN 9. The DM takes the party member with the highest Wits + Vigilance who has a score of +3 in total. The next highest score in the party is +1 but nobody else has anything. The total is +4 so the DM rolls for them and obtains a total of 8 -- that's not enough. As they begin to run, one of the PCs remembers they heard about a gnomish illusionist and asks `Are we chasing an illusion?' - that puts that final score up to 10; the TN is reached and the DM informs the player that she sees that the gnoll's feet are not always touching the ground properly, so it must be an illusion.

The DM should grant bonuses and penalties to illusions depending upon lighting conditions -- illusions inside a shadowy cottage seen from far away should receive an immense bonus, while far-fetched illusions on a sunny day seen up close might receive a penalty.
...
\spelllevel
\spell{Static Illusion}{Continuous}{Varies}

The illusionist can create one of two basic effects -- either a simple audio illusion or a static, single object or figure, no larger than a man. Illusions can be walls where there were no walls, or doors over walls or they could make the image of a sleeping guard dog.

Audio illusions include a shouting voice, an echo, a bear's roar or a bubbling brook; they must always be a single sound, not a plurality of voices. The voices must create simple, repetitive sounds such as a cry for help or shout -- not a full, dynamic conversation or a sound which appears to move.

Static illusions are extremely delicate and can be dispelled by a mere touch -- even a strong wind can scatter them, leaving the substance dissolving like a wilting flower before breaking into so much magical dust. The illusion cannot settle on moving things such as people because even breathing will shift the illusion apart, although characters can keep an illusion maintained over themselves without breaking it by keeping extremely still and breathing little -- this requires a TN 7 task with Dexterity.

\spell{Flashing Light}{Continuous}{Academics}
Additionally, the second level allows the illusionist to cast a bright light. It can light up any area, just as the Sun Ray spell under the second level of the Aldaron Sphere.

\spelllevel
\spell{Illusory Mask}{Continuous}{Varies}

The illusionist can make a mask to sit over moving objects, making one thing appear as another. The mask can also modify voices, allowing people to imitate another sex or race easily (but not to know a language), or even make a single person sound like a cacophony of voices such as a small army of soldiers shouting. It could also modify a weapon, making a sword appear as an innocent if clunky walking stick or a bow as a simple long stick.

Mask illusions are still rather delicate, and if someone's face is touched it will dissipate quickly, and there's no chance of maintaining a mask while wrestling or wiping one's face.

Many illusionists use this to disguise themselves as other people, although the target can attempt to see through the illusion with a +2 bonus if they are particularly familiar with that person.

Illusions can make things appear a little smaller than they are, but diminishing objects -- such as making a trident look like a sword -- always inflicts a steep penalty on the roll.

\spelllevel
\spell{Perfect Illusion}{Continuous}{Varies}

This powerful spell creates an illusion capable of full motion, varied sounds and intricate details. It functions largely as before, but onlookers receive a -2 penalty to disbelieve what they are seeing. Making the illusion act requires a modicum of concentration; when in combat, 3 Initiative must be spent.

\spell{Large Illusion}{Continuous}{Varies}
Large illusions do not have the detail of Perfect Illusions, but they are nevertheless impressive, covering up to 3 squares plus the illusionist's Wits Bonus.

Large Illusions have a basic Dexterity and Speed Bonus of 0, and the illusionist can add to this a number of points equal to 3 plus their Intelligence Bonus. The illusion also has all of the skills which the illusionist has (maximum +3). For example, and illusionist with an Intelligence Bonus of +1 could create an illusion with Speed +2 and Dexterity +2. An illusory dragon could be made to run at 12 squares per \gls{round}, or even fly at that speed. Of course, there is little use for too much speed, as any illusion which catches someone to attack would merely show its true nature as it dissolved upon contact with the target.

Once created, illusions have free range to roam as far as they please and remain for as long as the spell remains standing, though they will not do very much without periodic control from their creator.

Such illusions can also be separated creatures or items. An illusionist might make an entire dinner table appear as if varied food items were upon it, so long as the total covered no more than 3 squares plus their Wits Bonus in a continuous surface. Many illusionists attempt to make illusions out of themselves -- with so many copies people don't know which one to hit.

\spelllevel
\spell{Massive Illusion}{Continuous}{Varies}

The illusionist can now finally add the sensation of touch and smell to illusions. They are never solid enough to harm people, but are each tough enough to have 1 HP so attacks of 0 Damage do not dispel them. Touch allows a convincing illusion of a talking, interactive person, holding objects or creating things. The mage can assign a total of 4 points plus the caster's Intelligence Bonus to the illusion's Dexterity and Speed Attributes (maximum +4).

Such illusions can be made to fly or hover -- a giant, grabby hand might be created to grab a faraway object so long as it is not too heavy. A giant, flying dragon, complete with fire, might be made to fly down and terrify people.

These massive illusions can now cover 4 squares plus the illusionist's Wits Bonus and grant them a +2 bonus to resist anyone spotting them as they become more detailed, just as with Perfect Illusion.

\spell{Black Spot}{Continuous}{Varies}

Illusionists can also create illusions solid enough to cover people's vision without instantly being dispelled. Great black blotches of pure darkness can be made to cover an area, blinding anyone inside completely.

\spelllevel
\spell{Invisibility}{Continuous}{Deceit}

This ultimate level of illusion magic allows the caster to create negative illusions. This might include the illusion of a hole in the ground but more commonly is used to make things vanish. The caster, a tree or even a house can become almost completely invisible. The usual roll to disbelieve the illusion now becomes a roll to spot the invisible object by noticing little ripples in the air or footprints on the ground.

Anyone targeting the invisible must use the Blind Fighting rules. DMs may wish to reduce the penalty to strike the illusionist when the terrain helps vision, such as when the invisible creature must walk through water or across snow.

\sphere{Invocation}
\index{Invocation}This is the first choice of spheres for a battle-mage. It is designed specifically to destroy targets with balls of lightning and fire. It also has more subtle uses as casters can extinguish flames, plunging people into darkness.
...
All Invocation spells are rolled as Projectiles, using the mage's Intelligence Bonus and their Projectiles Skill; casters must have a Projectiles specialisation in invocation or receive a -1 penalty to all spells. The basic TN is 7 and the difficulty raises by +1 for every 5 squares away the opponent is, just as with normal missile weapons. As usual, opponents who are keeping edgy (see page \pageref{edgy}) can use their Speed to resist the attack, adding it to the TN. Alternatively, if a player is keeping edgy, it is they who can attempt to dodge the incoming attack, rolling their Speed at TN 7 plus the pyromancer's Intelligence and Projectiles Skill. Shields' Evasion Bonus can add to the roll to resist such spells.
...
\spell{Fire Flare}{Instant}{Projectiles}

Alternatively, the mage can make any nearby fire shoot out as a fireball and strike someone. A tavern's fire might explode, striking someone nearby, or someone's lantern-light could leap up and throw itself in the holder's face. The original fire instantly extinguishes, as the spell burns it up. The spell deals the usual Damage for a Lightning Bolt, modified by the size of the fire. A candle might inflict the normal Damage -2, while a bonfire could add up to 3 Damage to the spell.

The spell extinguishes the target fire.

\spelllevel

\spell{Fireball}{Instant}{Projectiles}

The mage sends out a ball of flaming death from his hands to incinerate a group of enemies in a wide burst. The total area covered is a ball of 2 squares plus the caster's Wits.

The mage rolls Intelligence + Projectiles, TN 5 (or the opposing player dodges at TN 9). If they miss the fireball flies off in a random direction not too far from the intended targets. Anyone one square away from the area of effect takes 1 Damage plus the mage's Intelligence bonus. Anyone in combat with someone in the fireball's radius counts as being `next to' the fireball.

\spell{Massive Flare}{Instant}{Projectiles}

This operates as Fire Flare, except that the size is 2 squares plus the caster's Wits Bonus.

\spelllevel
\spell{Flaming Arrow}{Instant}{Projectiles}

The mage learns a more potent version of Lightning Bolt and can deal $2D6$ Damage plus their Intelligence bonus with a flaming arrow of fire and lightning.

\spell{Eternal Flame}{Continuous}{Survival}

The mage sets a fire to burn eternally, with neither fuel nor end. An affected piece of woods might burst into flame and after the entire area had turned to charcoal it would continue to burn as a little flame in the middle. A patch of stone or the iron portcullis of a castle might be set to burn eternally. Whatever the substance, the spell affects an area but cannot target people, who can simply move away from the target area. Anyone inside that area suffers $1D6$ Damage plus the caster's Intelligence Bonus. The fire covers a total of 3 squares plus the caster's Wits Bonus.

Targeting people works just as Level 2: Fireball, the only difference is that the flames keep burning for as long as the spell is maintained. This is an effective way of burning down any wooden building, as a fireball spell can simply go out when a fire does not catch.

Of course, such fires can always be put out manually, with sufficient water.

\spelllevel
\spell{Lightning Storm}{Instant}{Projectiles}

A full storm of lightning is summoned to incinerate all opponents in a wide ball of destruction. This spell covers an area of four squares plus one square per Wits Bonus. The Damage is $2D6$ plus the caster's Intelligence Bonus and anyone next to the blast radius suffers the spell's minimum possible Damage.

\spelllevel
\spell{Molten Core}{Instant}{Projectiles}
...
The Damage is $2D6$ plus the caster's Intelligence Bonus.
...
The caster can destroy an existing spell, whether that spell is a persistent effect, such as a polymorph, or a magical item.  The TN is the highest Sphere level used plus the Intelligence Bonus of the \gls{miracleworker} who cast the original spell.

For instance, if a priest with Intelligence +1 and Aldaron +3 were to cast a spell enchanting an animal, the TN would be ($7+3+1=1$) 10.  The caster would roll using Intelligence plus their rating in the Aldaron sphere.

\spell{Spell Throwing}{Instant}{None}

Throwing a spell means that the normal range boundaries can be ignored -- spells can be thrown anywhere. Of course, the target must usually be seen -- there's no use casting a flame bolt into the darkness of a forest and specifying the target as `any nearby enemies'.
...
Additional spells may be placed on items which are there simply to act as a trigger.  

\spell{Spell Forking}{Instant}{None}

Each spell must target different people -- a doubled spell targeting the same person is not mirrored but is just a single spell. Spells cast in this way can have different effects, such as a conjuration spell conjuring a range of random animals, but they cannot cast different spells, such as multiple spells in the second level of aldaron being cast. Such spells cannot successfully execute a Vitals Shot -- the mage cannot make such precise magical strikes when targeting so many people at once.

Spells cast at a higher level require the usual Initiative cost of that higher level, so a Necromancy spell cast at 4th level would have an Initiative cost of 7.

Not all spells gain any use from additional targets. In particular, spells which target only the caster cannot be cast at multiple targets. Mana Stones cannot be cast at a higher level to increase the number of mana stones created.

If any spells cast with Mirror Magic are made into standing spells then each one remains standing with the cost of maintaining only the original spell. For example, a level 1 force sphere spell might be cast to create Mage Armour, granting 4 \gls{SP}. If the mage has Intelligence +1 and Wits +1 and cast the spell at an effective level of 2 then they could grant the same shield to up to 3 people and maintain it for the cost of just 1 \gls{mp} because only a first level spell is being used.
...
The mage can finally create their own zombies from easily accessible realms of malicious spirits. The spell is cast on a corpse and the corpse is imbued with one such malicious spirit. It retains the Strength score (and therefore HP) it had in life. The corpse has Dexterity, Speed and Wits scores of -2 -- it can run, but not terribly quickly. The creature has neither intelligence nor Charisma scores. Most will attack all living things on sight.

The mage rolls their Intelligence + Medicine at TN 7 to cast the spell. Any Medicine specialisations dealing with the affected species (e.g. `gnolls', or `humans'), or specialisations concerning death rituals can be used.

Only humanoid corpses can be summoned back from the dead at this stage. Elves are difficult to use as their corpses degrade particularly quickly, often turning to dust within a week. Gnolls, humans and nura of all manner make powerful undead warriors.
...
\spelllevel
\spell{Greater Undead}{Instant}{Academics}

The mage can now summon spirits from any realm of dead spirits. This is sometimes used to summon powerful spirits from strange realms, and at other times used to bring back the souls of dead people to hang spectrally in the air while the mage asks questions. In both cases, the spirits in question can be summoned simply to speak, or alternatively placed into a body.

The necromancer rolls their Intelligence + Academics at TN 9. Unwilling people can resist the call to bring them back with their Wits Bonus and Deceit Skill. If placed in a body, most returning spirits of the dead go mad with despair at the lack of normal sensation and find a way to end their own existence before long. Spirits who never had a body, those who who wander as disembodied things in the planes beyond, can fare better for short periods, but not much longer. Disembodied spirits are difficult to see, and almost impossible in full sunlight as strong light cuts straight through them. When asked about the afterlife, souls will have different responses, depending upon their deity of choice. Those who never worshipped a god always find themselves wandering alone in a vast and strange afterlife, with dragons and demigods, often hounded but always free to make their own choices.

To avoid madness and to ensure obedience, necromancers summon foul souls from strange bowels in the spirit world. Mages can specify what kind of spirit they wish to summon and then assign exact Traits.

\index{Spirits}Spirits are assumed to have Dexterity 0, Speed 0, Intelligence 0 and a Wits score equal to the mage's. Their Charisma score begins at -3. The mage can add to these stats a total equal to 3 plus their Intelligence Bonus. Racial maximums are still in effect and the undead creature's Strength Attribute is, as per previous levels, determined by the corpse. Spirits have a hard time interesting themselves in anything other than eating souls, but they can gain Skills in the normal way up to a maximum equal to their Intelligence bonus. Spirits summoned with Intelligence +2 could gain up to the second level of any Skill.

\begin{figure}
	\begin{tcolorbox}[arc=1mm,tabularx={|cp{0.68\textwidth}}]

	TN & Effect \\\hline

	+Wits + Deceit & Targets who do not wish to be summoned add their Wits bonus to resist the spell. \\

	+2 & The target has been dead more than a century or has never been a living person. \\

	+1 & The target has been dead more than a decade. \\

	+2 & The target's name is unknown. \\

	-2 & The target's body is present. \\

	-2 & A personally important item to the target is present. \\

	-3 & The target has pressing, unfinished business on earth or has a great desire to simply come and wreak havoc. \\

\end{tcolorbox}
\end{figure}

At this level, all kinds of animals can also be summoned as undead -- bears, horses, stranger creatures -- anything no taller than a man.

Necromancers often hunt for powerful souls, researching their likes and original names in order to summon them back. Many historians use pseudonyms when discussing powerful kings and especially when discussing powerful magic-users so that they cannot be summoned back from the dead by a mage to use their abilities at their service.

When summoning a specific soul back from the dead, that soul comes with all their original skills and magic spheres. However, the mage must be able to give them the same Mind Attributes as they had in life. Intelligence and Wits Attributes can be lowered if need be, but if the summoned spirit must have Intelligence +3 and the mage cannot reach this minimum level then the spell fails.

\spelllevel
\spell{Death Sight}{Continuous}{Medicine}

The mage learns to use the death sight of the undead -- while the spell is active they can see fine in the dark, just as the dead do. This spell also `contains' all of the first level, including the \gls{dr} of 1 and the resistance to Fatigue Points. This level of undeath can be cast upon people, even unwilling people. Targets not used to such a deathly touch often flee -- an immediate morale check is taken at TN 12 to withstand the horror of seeing people's souls while their veins immediately run slow and cold. The horror check can be taken with the user's Wits score and either Combat or Academics; academics are often less perturbed by strange ideas, having become used to the strangeness in the world through an open mind.

The spell merely requires an Intelligence + Medicine roll at TN 7, just as with level 2. Unwilling targets have no way of resisting this spell. The Charisma penalty for this spell raises to -4 as the target is further touched by a horrifying aura of death.

\spell{Colossal Undead}{Instant}{Academics}

The mage can also summon any creature, no matter the size, from the dead to act as a servant. Umber hulks, dragons -- literally any corpse can have a malicious soul implanted within it. These souls function as per Greater Undead (above). The maximum Strength Bonus of such creatures is 4 plus the mage's Intelligence Bonus -- usually enough to raise any creature from the dead.

\spelllevel
\spell{Touch of Death}{Continuous}{Medicine}

The mage can finally assume all abilities of the undead -- not simply death sight. They gain a \gls{dr} of 2 which stacks with armour and is immune to Fatigue points. At the end of each day the spell is active, the mage makes an unmodified Charisma roll at TN 7; failure indicates death. Not that this will stop the necromancer, it simply means they are now a member of the undead and this spell can be safely cancelled. All Charisma modifiers become permanent and now affect FP totals.

\spell{Hearth Attack}{Instant}{Medicine}

The mage can also reach out and wisp away a target's essential life essence with a spell, draining all heat and vitality, bringing even the boldest hero to death with a word. The spell causes $1D6$ plus their Intelligence Bonus in HP Damage -- it ignores \gls{fp} \& \gls{SP} entirely. It ignores all armour. No roll is made, the target simply needs to be in range and immediately they start to die.

\sphere{Polymorph}
\iftoggle{verbose}{
\begin{wrapfigure}{R}{.37\textwidth}
\includegraphics[width=.37\textwidth]{images/Roch_Hercka/polymorph.jpg}
\end{wrapfigure}
}{}

\index{Polymorph}The Polymorph sphere of magic allows the mage to grasp at different strands in the tree of life, grasp at other forms of life and shift their body along a different branch, transforming into an animal of some kind or some other race. At later levels the mage can turn into stranger things still -- elemental forms such as pure air or a raging ball of fire.

Throughout all these forms people maintain a universal `face' -- a kind of likeness which they simply cannot get rid of. Many conjecture that the face is a facet of one's soul showing in the world. A ginger person transformed into a cat would become a ginger cat. A skinny person with short hair who transforms into a sheep will become a skinny, short-haired sheep. Spotting someone who has been transformed requires a Wits + Empathy roll, with a TN of 8 plus the level of the polymorph sphere being employed; e.g. if an elf used the first level to transform into a gnome the TN would be 9, but if the elf used the fifth level to transform into a magma elemental, the TN would be 14.
...
For example, Meldon the elf has 5 HP. He takes 3 HP Damage and already has 3 Fatigue Points leaving him with a -1 penalty to all actions. He then transforms himself into a bird, lowering to 2 HP. He now has zero Damage but retains his -1 penalty due to Fatigue. After flying away to safety he rests for a while and heals all his Fatigue Points, but when he turns back into an elf all his old wound reappear as his HP increases to the point where they can affect him.

Finally, the alternate forms someone adopts while polymorphed disrupts one's magical abilities, inflicting a +2 penalty to all spells cast while polymorphed. This penalty can be ignored with sufficient practice in an alternate form, but it requires at least ten years of persistently holding the altered form.

\spelllevel
\spell{Minor Polymorph}{Continuous}{Medicine}

The skin shifter starts off by accessing branches close to them on the tree of life, changing race from one humanoid type to another. A gnome might become an elf or a human might become a dwarf. In each case the Physical Attributes alone change to match the new race. Since humans have Strength +1 and gnomes have Strength -2, a human transforming into a gnome would lose 3 points of Strength while a gnome transforming into a human would gain 3.

The roll for this spell is Intelligence and the Medicine Skill. The basic TN is 7 and every HP increase or decrease the mage wants to affect will increase the TN by 1. For example, transforming from a human (with a +1 bonus to Strength) to a gnome (with a -2 penalty to Strength) would mean a TN of 10.

Larger and stranger creatures, such as dragons, can use this ability normally, though the large strength difference between themselves and the demi-humans can make turning into, e.g., a gnome, difficult. At no point can someone transform into a race theyare unfamiliar with.

Polymorphing into another race is never easy - the body simply doesn't feel right.  All Physical actions suffer +1 to their TN while the caster is Polymorphed.

\spelllevel
\spell{Polymorph Self}{Continuous}{Beast Ken}

	\begin{tcolorbox}[arc=1mm,tabularx={lrrp{.52\textwidth}}]

	Animal & Str. & Spd. & Special \\\hline

		Badger & -2 & 0 & \glsentrytext{dr} 2 \\

		Bear & +5 & +3 & \gls{dr} 3, +1 Dam after grapple. \\

	Beaver & -3 & -1 & +3 Speed in water. \\
	...
	Frog & -5 & -4 & Swim \\
		...
		Goat & -1 & +2 & +2 Dam, \gls{dr} 2, +2 to climbing. \\

	Horse & +1 & +4 & \\

	Large Cat & +2 & +4 & +3 Dexterity to climb \\

	Pig & 0 & 0 & +2 \gls{dr} \\

	Rat & -5 & +2 & Disease immunity \\

	Wolf & -2 & +4 & +3 Dam after grapple \\

\end{tcolorbox}

With more advanced training, the shapeshifter can change their entire body into the lower forms of the beasts, with only limited restrictions -- they can reduce weight down to a fifth of the normal size, becoming a wolf, badger or large cat, or can increase up to five times as large, transforming into a small bear or larger cat. Characters who are large and muscular will have an easier time turning into a bear, while shorter, faster characters will find it easier to turn into a bird.

The player rolls Intelligence + Beast Ken at a TN of 7 plus the number of HP they are changing. If successful, the character can raise or lower a total number of Attribute points equal to 2 plus their Intelligence Bonus in order to raise or lower Strength, Speed or Dexterity. Characters must bring their Attributes within 1 point of the Attributes of the creatures they are changing into.

For example, a strong priestess of Laiqu\"{e} attempting to change into a bear might have Strength +3, Speed +2 and Dexterity +1. A bear has a +5 listed in Strength, meaning to transform into a bear someone must have a Strength Bonus between +4 and +6. The listed Speed Attribute is +3 so bears can have anywhere from +2 to +4 in Speed. The character raises her Strength from +3 to +5 then raises Dexterity to +2. Now that the Attributes are in line with what a bear should have, the transformation is finished and the caster turns into a bear.

Later, the priestess wishes to turn into a bird. She must lower their Strength from +3 to -5. Unfortunately, this requires their Strength to lower by 8 steps and she can only lower it by 3 steps, so the spell is impossible. Others, such as an elf starting at Strength -1, will only have to lower their Strength by 3 in order to attain Strength -4; this is within 1 step of a bird's listed `-5' Strength, so this is acceptable. The elf would then only need to reach up to Speed +4, because a bird must have a Speed Bonus between +4 and +6.

In all cases, the caster can raise or lower their Dexterity by any amount and this will not restrict what animals they can turn into.

Certain animals come with some Special ability. Some are tough, others have bonuses to certain actions such as swimming or flying.

Spells cannot handle partial transformations -- they either succeed or fail entirely. If the mage wants to transform their clothing along with the spell, they need a Margin of +1. Each additional large item (anything with a weight of -1 or more) requires an additional Margin, so transforming while wearing plate armour and carrying a shield and sword would require a Margin of 3. If the Margin is not reached, the clothing either bursts or remains on the ground after the transformation. If the Margin is successfully achieved then the items disappear, only to return after the shapeshifter has transformed back. People in animal forms cannot use tools such as weapons as animal hands do not have the necessary digits.

\spelllevel

\spell{Polymorph Other}{Continuous}{Beast Ken}

This functions as above, but can be used to Polymorph other targets into animals. The mage rolls their Intelligence + Beast Ken and if the target is unwilling, they resist with their Intelligence + Deceit Skill as the mage attempts to understand and thereby shape the target's form.

When casting this spell the mage instantly knows all of the target's Body Attributes in order to make a decision about what kind of animal the target might best be transformed into. The mage can now rearrange or add up to 3 points of Body Attributes plus their Intelligence Bonus.

For example, Darren the priest of Alass\"{e} notices a guard organising bets on who can kill a rat fastest. Disgusted by the needless deaths, James decides to make an example of him. James has Intelligence +1 so he can raise or lower up to 4 Body Attribute Points. The guard has Strength +1 so he cannot be turned into a rat himself. James decides to turn him instead into a badger as it's the closest thing available. Badgers have a listed Strength -2 and Speed 0 so the druid decides that Strength -3 and Speed -1 would be better. The guard's Intelligence is 0 and his Deceit Skill is +1, and since the guard is losing 3 points of Strength, he also loses 3 HP; this raises the TN by a further +3 for a final TN of 11. The holy man succeeds on the roll and to the horror of all the onlookers, a badger emerges from a ruffled pile of cheap tunic and rusted chain shirt.

Transforming a target's clothing or other items requires a Margin, just as with Level 2.

\spelllevel
\spell{Greater Polymorph}{Continuous}{Beast Ken}

The mage can now induce partial transformations, essentially `making up' anything they like in terms of final form. The mage might transform them into a minotaur with incredible Strength or a very large mage might become a toughened bat-like creature. The mage can also modify others, making them into monstrosities or gargantuan killing machines.

The mage rolls Intelligence + Beast Ken and as with level 3, if the target is unwilling they resist with their Intelligence + Deceit. As usual, each HP the caster wishes to change increases the TN by 1. If successful, the caster gains a number of polymorphing points equal to 4 plus their Intelligence Bonus. Each point can be used to raise or lower a body Attribute as usual or can also be used to add features, including the following:

Adding any feature reduces the shapeshifter's pool of Attribute points by 1, so a shapeshifter with Intelligence +2 might elect to have wings, \gls{dr} 1, decrease Strength by 2 levels and then add 2 levels of Speed.

	\begin{tcolorbox}[arc=1mm,tabularx={lp{.7\textwidth}}]
		\textbf{Modification} & \textbf{Description} \\\hline

		\textbf{Natural Armour} & Shapeshifters can gain natural \gls{dr}. Each point \ sacrificed grants +1 armour, up to a maximum of 2. This stacks with armour, but the armour must be specially made in order to be wearable by whatever strange form the shapeshifter has chosen. \\

		\textbf{Natural Skill} & The creature gains +2 when performing some particular task such as climbing, swimming or flying. This may be selected up to 3 times. \\

		\textbf{Wings} & Creatures can only use wings to fly if their Speed Bonus is 6 points higher than their Strength. They can glide if their Speed Bonus is 4 points higher than their Strength. Otherwise, the wings are useless. If wings are used as shielding, they count as weapons with -2 Damage, +2 Initiative and +3 Evasion. If the wings successfully shield the target from attack then the target still receives full Damage but the maximum HP loss is 2. Once the target has lost these 2 HP the wings are destroyed and no longer provide cover. \\

\end{tcolorbox}



Taking clothes and other items into one's form when transforming works as per level 2.

Creatures with such advanced levels of shapeshifting ability often lose their own race, becoming generic creatures in their own right, electing to transform themselves each day into stranger and more wonderful forms. Eventually, such generic humanoids change their racial maximums and minimums to the range of -4 to +4. When in this generic form the casters appear as grey, almost featureless creatures.

\spell{Plant Polymorph}{Continuous}{Survival}

Additionally, mages of this level can transform themselves into any plant they are familiar with: trees, bushes and more are all possible. While in this form, they can be exceedingly difficult to destroy, but are, of course, completely immobile. They have no innate ability to sense their environment beyond a sense of touch, so without further magical assistance they cannot cast spells at others. The plants gain a \gls{dr} equal to double the mage's Strength Bonus as Perfect Armour.

\spelllevel
\spell{Elemental Polymorph}{Continuous}{Academics}

With the final level of Polymorph, the mage can take on an elemental form, turning into pure animated fire, or a fiery humanoid; an ice-man with hardened skin or a whispy air spirit.

The caster rolls Intelligence + Academics while any unwilling targets resist with Intelligence + Deceit. If successful, they gain a total number of points to play with equal to 5 plus their Intelligence Bonus. These additional points can be used just as level 4 of the polymorph sphere and two such points can also be spent to gain any of the following abilities:

Modification Description

Perfect Armour\ The mage gains hardened skin, granting +3 \gls{dr}. This is cumulative with armour, if any appropriate armour can be found to fit this strange new form. This cannot be taken at the same time as the \gls{dr} bonus of Polymorph level 4.

Burning Touch\ The creature's tough is particularly dangerous, perhaps due to exuding acid or maybe the creature is permanently on fire. The creature is immune to its own Damage type, even from other sources. The creature inflicts +2 Damage on brawling attacks and always inflicts normal Damage, never Fatigue Points.

Ephemeral\ The creature's body has little to no substance -- attacks pass straight through them, dealing little to no Damage. They cannot pick up objects or Damage people with a touch but also gains a \gls{dr} of 10 which counts as Perfect armour. It is still possible -- if barely -- to scatter their body with enough force. Ephemeral creatures can float slowly, up to 1 square per \gls{round} in any direction, including upwards. While floating, they are subject to being moved by the wind. If they have the Burning Touch modification (above) they can use it to cause $1D6-2$ Damage upon a successful attack. Creatures with this modification cannot take any additional \gls{dr} from any other source.
...
\section[The Path of Alchemy]{The Path of Alchemy}
\index{Alchemy}\textbf{Spheres}: Conjuration, Invocation, Force, Illusion, Metamagic
...
\noindent The alchemist learns magic through rote repetition and formulae which are usually be invoked through precise hand-gestures and mystical words which are attuned to the background harmonics of the universe. Without the ability to move one's hands and use one's voice, alchemy spells take a -2 penalty to any task roll or a -4 penalty if the mage can neither move nor use their voice. Alchemy was invented by the gnomes but has since become popular with various upper-class humans. This is the typical magic of a standard town wizard. Alchemy requires one slot of Academics in order to be learnt.

Spells summoned by the Path of Alchemy are accompanied by magical sparks and sometimes loud bangs. Their mana stones are always based on precious minerals or rocks such as rubies, sapphire or even diamonds. A mana stone costs 10 gp per \gls{mp} which can be stored inside it, so a mana stone storing 3 \gls{mp} would cost 30 gp. The exact item might be a simply ruby which stores mana, a diamond-headed wand of ivory which blasts out fireballs or a sword with jewels on the handle which surrounds the warrior with moving illusions of their. Alchemical mana stones with a spell always activate those spells with a command word.

Alchemists who are trying to hide the fact that they are casting a spell must use a Ritual Spells. The roll takes a -2 penalty (if indeed there is a roll). Onlookers suspicious of the use of magic can make an opposed Intelligence + Academics roll (using the alchemy specialisation) while the mage uses Charisma + Deceit.

Alchemists cannot naturally intuit how the next level of any sphere works. Instead they must pick up levels slowly and through intense study. They only receive new levels during \gls{downtime}, and must pass an Intelligence + Academics roll at TN 12. Proper studying material will add a +1 to +3 bonus to this roll. Each week of \gls{downtime} might grant one roll to learn a level of some sphere.
...
The path of devotion requires casters to both use their voice and to move their hands, as per Alchemy. Failure to do either results in a -2 penalty. So using one's hands to wield a weapon while being underwater would give a -4 penalty to any spells cast.

\section{The Path of Runes}
\iftoggle{verbose}{
\begin{wrapfigure}{R}{.5\textwidth}
\includegraphics[width=.5\textwidth]{images/Roch_Hercka/dwarvish_runes.jpg}
\end{wrapfigure}
}{}
...
\noindent The character has learnt the magic of song. They can sing illusions into existence, inspire people with great tales and enchant people with a lute. Any instrument, song or performance suffices for casting a spell so long as it is appropriate -- a flute is not a good way to magically make people scared.

A bard casting spells through a battle can sound like an ever-changing single song, now going faster, now singing of blessings, now an enchanting and calming lilt. The first spell cast takes an additional 2 Initiative points to cast but each spell cast after that costs 1 less initiative. Activated spell-songs, usually costing 8 Initiative, are also reduced in cost. A bard might cast a blessing on their companions with the second level of the fate sphere, paying 7 Initiative instead of the usual 5. After that they want to cast an illusion of them -- normally this would require 4 Initiative as this is a first level spell, but it now costs only 3.
...
In order to learn the Path of Song, the mage must have the second level of the Performance Skill. The mana stones of the Path of Song are actual songs. The bard composes a song especially for the purpose; when anyone -- anywhere in the world -- plays the song on the correct instrument the mana can be regained. Such songs can also become vessels for spells, becoming magical items which activate once played.
...
You are particularly difficult to flank. So long as you are not surrounded on all four sides you receive only a -1 Evasion penalty for each person attempting to flank you, up to a maximum of -4.
...
This knack is automatically granted by using a medium sized shield, so anyone who both has the Knack and a shield could spend 1 Initiative point at the start of the \gls{round} to be able to dodge all incoming missile attacks. If their Speed were +1, they would gain a +4 bonus to dodging, or anyone attacking them would raise the TN to hit this character by 4.

\subsubsection{Expert Flanker}
The character can irritate and distract multiple people at the same time. Each Knack they have allows them to flank an additional opponent. They also reduce the target's Evasion scores by an additional 1, so if their Strike were +2 they could lower nearby opponents' Evasion Factor by 3. This Knack cancels all effects of the Knack: Back to the Wall for anyone in range of the character.
...
You gain +2 to Strike when using the Aggressive Stance.
...
Multiple purchases of this Knack allow you to bypass armour at an increasingly low TN.
...
You are particularly adept at targeting weapons' weakpoints. When using any weapon with a Damage bonus of +2 or greater you can make an attack against an opponent's weapon instead of the opponent. The TN is the same as hitting the opponent +2. The attack must cause at least 6 Damage. If successful, blade weapons shatter and count as daggers, giving nothing more than +1 Damage. All other weapons which are successfully destroyed are simply gone, providing no bonuses at all. Certain good quality weapons are immune to this effect.
...
This Knack may only be bought at character creation or immediately after the caster is required to take a Vitality Check to avoid death. It may not be taken while following Laiqu\"{e}, Alass\"{e} or V\'{e}re.
...
\begin{xpchart}{Alass\"{e}}
	...
	3 & Creating a funny song. Requires at least a full night and an Intelligence + Performance action, TN 10. \\
...
\begin{xpchart}{C\'{a}l\"{e}}
	...
	10 & Writing an informative book on some topic. Intelligence + Academics is rolled at TN 12 during downtime. \\
...
\index{Gods!Laiqu\"{e}}\subsection[Laiqu\"{e} -- Goddess of the Forest]{Laiqu\"{e} -- Goddess of the Forest}(`Lie-queh')
	...
	3 & Building a shrine -- requires 3 days work and an Intelligence + Crafts action, TN 8. \\
	...
	5 & Finding a new type of creature. \\
		...
		10 & Composing a song to Laiqu\"{e} -- requires an Intelligence + Performance action, TN 10. \\
...
\begin{xpchart}{Qualm\"{e}}
	...
	3 & Erecting a shrine to the dead. Requires an Int + Craft action, TN 9, at at least 10 gp. \\
...
Followers of Vére who break any deal cannot gain \gls{xp} until they atone for the crime.
...
Vére's clerics can access the enchantment and force spheres. They use enchantment to gain followers, dazzling them with the glory of the purity and strength of their god while force is used to protect the innocent and faithful. Their spells appear in a shimmer of gold. Vére's mana stones are always people who are followers of Vére. Those believers alone can activate any spells which are stored inside them. Priests of Vére often gift their followers with single-use magical powers, such as the ability to call upon a blessing or the ability to protect themselves with armour. If the people who are being used as mana stones are given spells then they can activate those spells at will with a short prayer at an Initiative cost of 8.
...
\begin{xpchart}{the Code of Experience}
	...
	5 & Creating a masterpiece of some kind at TN 12. Each type of thing can only be attempted once to gain this \gls{xp}. \\
