...
During the grappling rolls, the two make an opposed roll of double their Strength Bonus, plus their Combat Skill. The \gls{tn} to maintain the grapple is 6, at which point Fatigue Points are applied just as with a Brawling roll. If the Margin is 3 or more then normal Damage is applied instead of Fatigue. If targets are wrestling while using a dagger then this Dagger deals Damage as per usual on any successful roll, no matter what the Margin.
...
\subsection{Casting}
Spells are cast by spending a number of \gls{mp} equal to the spell's level, so 1st level spells always cost 1 \gls{mp} and 3rd level spells always cost 3 \gls{mp}.  The character then makes a roll against some \gls{tn}, and the spell is cast if successful.
...
Forest's Friendship works on all creatures without an Intelligence score. Umber hulks, bears, birds, et c. - all can be affected with the language of the forest. However, mammals are the easiest to work with. The DM should add to the \gls{tn} to affect birds, insects and other non-mammalian creatures.
...
The caster changes any solid target to stone.  The \gls{tn} is 7 plus the target's Weight Rating.\footnote{A living target's Weight Rating is equal to their \glsentrytext{hp}.}  Once the spell is over, the target turns back to normal.
...
The first leve's enhancements can be added to this level, so a mage could create an \textit{Enlarged Artefact} at third level, and summon an entire wagon.
...
The caster can turn water into ice, or air into stone, or otherwise make any barrier.  The barrir takes up up to two squares plus the caster's Wits Bonus.  Such barriers can only be broken with a Strength + Crafts roll, \gls{tn} \setcounter{enc}{7}\addtocounter{enc}{\value{spelllevel}} \arabic{enc} plus the caster's Intelligence Bonus.

\spell{Prison}{Continuous}{Crafts}

The caster freezes the water around the target, or turns surrounding air to stone, imprisoning them.  While the spell is being cast, the target can attempt to break free as a Quick Action, costing 2 Initiative, by rolling Strength + Athletics.  The \gls{tn} is 7 plus caster's Intelligence Bonus plus Crafts.
...
If the \gls{miracleworker} can cast the spell quickly, rocks can be conjured above a target's heady, dealing $1D6 + \arabic{spelllevel}$ plus the caster's Intelligence Bonus in Damage.  However, in such cases targets who are Keeping Edgy can resist with their normal Evasion score, which increases the \gls{tn}.
...
For example, a gnome creates an illusion of a fleeing gnoll with a great bundle of treasure in his hand, hoping the PCs will chase after him immediately. His Intelligence is +2 and his Academics is at +1 though he has no appropriate specialisations, so the players are rolling at \gls{tn} 9. The DM takes the party member with the highest Wits + Vigilance who has a score of +3 in total. The next highest score in the party is +1 but nobody else has anything. The total is +4 so the DM rolls for them and obtains a total of 8 -- that's not enough. As they begin to run, one of the PCs remembers they heard about a gnomish illusionist and asks `Are we chasing an illusion?' - that puts that final score up to 10; the \gls{tn} is reached and the DM informs the player that she sees that the gnoll's feet are not always touching the ground properly, so it must be an illusion.
...
Static illusions are extremely delicate and can be dispelled by a mere touch -- even a strong wind can scatter them, leaving the substance dissolving like a wilting flower before breaking into so much magical dust. The illusion cannot settle on moving things such as people because even breathing will shift the illusion apart, although characters can keep an illusion maintained over themselves without breaking it by keeping extremely still and breathing little -- this requires a \gls{tn} 7 task with Dexterity.
...
The roll for this spell is Intelligence + Medicine Skill. The basic \gls{tn} is 7 and every HP increase or decrease the mage wants to affect will increase the \gls{tn} by 1. For example, transforming from a human (with a +1 bonus to Strength) to a gnome (with a -2 penalty to Strength) would mean a \gls{tn} of 10.
...
Polymorphing into another race is never easy - the body simply doesn't feel right.  All Physical actions suffer +1 to their \gls{tn} while the caster is Polymorphed.
