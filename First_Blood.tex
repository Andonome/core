...
\tcbox[tcbox width=minimum right]{testing}
...
The Polymorph sphere of magic allows the mage to grasp at different strands in the tree of life, and move their body along different paths.  Nearby forms include other races, such as elves turning into men, and later shapeshifters learn to turn into bears, hawks or other animals.  Larger men find it easier to turn into large animals such as bears, while smaller creatures find it easier to turn into smaller animals, such as birds or badgers.

Master shapeshifters learn to go beyond the great tree of life and turn into arbitrary forms of their chosing, including living fire, or a gust of wind.
...
% Enhance: plantform
% Enhance (1): ignore difficulty levels for others.
% Enhnace (2): ignore difficulty levels for species.
% Enhance (2): spend Attributes on abilities, like wings.
% Enhance (3): spend Attributes on abilities
...
The basic polymorph spell allows the shapeshifter to add or subtract a number of points of Strength from the target.  This modification must bring the target up or down to the level where they are in line with the target's racial maximums and minimums.  An elf with Strength -1 could turn into a human or gnome easily, but turning a human with Strength +3 into a gnome would require reducing their Strength to +1, but gnomes have a maximum Strength of +1.

Changing one's own form is \gls{tn} 7, while changing another's is \gls{tn} 10.

The polymorphed target always takes a -1 Dexterity penalty due to the new form seeming strange, until they have had time to get used to it.\footnote{Any amount of downtime is a reasonable amount of time.}

Polymorphing into another race does not grant any of its racial abilities.  Changing one's shape to look like an elf will not grant cold-immunity, and polymorphing into a dwarf will not allow one to stomach strong drink.
...
Such animal transformations are in shape alone, and do not grant any abilities.  Such birds cannot fly,

Alternatively, the polymorpher can turn an animal into a person.  This won't yeild any fantastic results, as animals don't suddenly become intelligent once turned into a gnome or dwarf, but it is possible.  Such creatures start with Intelligence -5 and Charisma 0.\footnote{The GM is encouraged to change the basic Charisma for various animals, as cats obviously start with +3.}

This spell is cast at \gls{tn} 12, as it either targets an animal, or makes someone into one.

\enhancement{1}{Empathetic}

Advanced shapeshifters can extend a little mana into their understanding of alternate forms, and discard the usual \gls{tn} restrictions.  All \glspl{tn} become 7.

\enhancement{1}{Bolstered}

While basic shapeshifters can add to or subtract from the Strength Bonus of a target, a \textit{Bolstered} spell allows the caster to add or subtract a number of points equal to the spell level plus their Intelligence Bonus.  This allows a much greater range, so shapeshifters can turn themselves into birds, or bears.

\enhancement{1}{Realistic}

Shapeshifters taking on the basic form of another creature gain no abilities from it.  The \textit{Realistic} enhancement allows them to take on the lot.

Various animal forms have features which can be bought at the cost of one of the Attribute shifts.

\enhancement{2}{Freeform}

The shapeshifter can throw off the limits of existing and known creatures, and turn into anything they might imagine.  Any points which might otherwise have added or subtracted from Attribute points can be spent on special abilities instead.

% 1 ephemeral
% 1 dangerous - 1D6 damage per round of contact
