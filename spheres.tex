...
\iftoggle{verbose}{%
For example, if someone wielded a club (with weight rating 1), defending herself with Dexterity +1 and Combat +2, the TN for the spell would be ($7 + 4 = $) 11.
}{}
...
\iftoggle{verbose}{
Spells can now turn targets into complex forms, i.e. those composed of multiple substances, or with complex shapes.
These include:

\begin{itemize}

	\item{Acid (see the level 2 spell for details)}
	\item{Blade weapons (assuming the spell is also \textit{Metallic})}
	\item{Equipment for horses}
	\item{Rope}
	\item{A statue}
	\item{A table}
	\item{Wine}
...
The caster can now transform targets into detailed forms.
Air can become a complex, and rich scent.
Solid wood can turn into a sword, or rope.
Water can become beer, wine or even acid.
}
...
\spell{Summon}{Continuous}{Varies}\\
The mage can summon materials from nothing.
This functions as per the previous level, but no target is required.

\spell{Acid}{Continuous}{Academics}\\
Acid can be devastating when it hits skin, but does very little to hurt someone covered in even basic clothing.
If the acid achieves a Vitals Shot, it gets around armour or clothing, and deals $1D6$ Damage plus the spell's level, plus the caster's Intelligence Bonus, otherwise it deals half damage

If the acid is held in a tough bowl, made or metal or dense wood, it can be thrown like a normal projectile, so long as the range is short (-2 penalty per square's distance).
If the spell is used as a level 2 \textit{Summon} spell, the mage rolls Intelligence + Academics to hit the target, who can dodge as per a standard ranged attack.

\spell{Prison}{Continuous}{Crafts}\\
The caster summons ice surrounding a target, or turns surrounding air to stone, imprisoning them.
While the spell is being cast, the target can attempt to break free as a instead of a movement action by rolling Strength + Athletics vs the caster's Intelligence + Crafts.

\spelllevel

\spell{Teleport}{Instant}{Academics}\\
The mage teleports the target a short distance -- a number of squares equal to the spell's level plus the caster's Wits.
As with many other `insta-kill' spells, the target can cancel the spell by spending 5 \gls{fp}.%
\footnote{This is not because any teleportation can kill someone, but because the target could \emph{potentially} die from a teleport spell, putting them over a cliff, or similar.}

\enhancement{1}{Gated}
\index{Spell Enhancements!Gated}
The mage can not simply teleport something but open a doorway from wherever the mage is to somewhere within range.
...
If cast as a \textit{Range, Gated, Teleport}, the far portal can appear anywhere the mage can see.
If cast as a \textit{Wide}, or \textit{Massive} spell, the two portals can appear anywhere within the targeted area.

\enhancement{1}{Unrestrained}
\index{Spell Enhancements!Unrestrained}
The mage can ignore all spacial barriers to the teleportation spell, allowing it to go anywhere the caster is familiar with.
