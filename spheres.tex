...
\spell{Forest's Call}{Continuous}{Deceit}{Mark someone for a monster encounter}\\
\label{forestsCall}%
The caster makes a call to the forest to come and attack the nearby target.
If the target is a player, the \gls{gm} rolls \arabic{spelllevel} times plus the caster's Intelligence on the local encounter table, and the \gls{pc} faces all encounters within the next day, and typically within the next scene.
...
\iftoggle{verbose}{%
When cast as a \textit{Wide} spell, it forms an impenetrable barrier in a hallway or tunnel, by simply looping one place to another: once a portal blocks the way, nothing can get past it, because hitting the portal does nothing more than throwing fists into the other side.
...
When cast as a \textit{Pocket Spell}, a \textit{Ranged Gate} can retain its original location, no matter how far away the scroll travels once activated.
\iftoggle{verbose}{%
...
\iftoggle{verbose}{
  ...
  Even if a conversation does not end, the target would simply need to spend 2 Initiative points to speak a little while fighting.
}{}

Targets enchanted to continue dancing, sewing, or anything else, can make a second roll to break free of the enchantment as a Quick Action, costing 2 Initiative.
