...
\begin{itemize}
  ...
  \item
  \textbf{The \glsentryname{weight}}
  tells you how many \glspl{ap} you need to swing the weapon.
  \shortsword\weaponName s
  only provides a \absNum{weaponBonus} Bonus to hit, but also only need \arabic{weaponWeight}~\gls{ap} to attack.
  \greatsword\weaponName s
  provide a \absNum{weaponBonus} Bonus to hit, but costs \arabic{weaponWeight}~\glspl{ap}.
\end{itemize}
...
depends on the local metal supplies, and the skills of the local \gls{armourHall}.
...
\begin{exampletext}
  ...
  {\sffamily ``Gimme a group roll of \roll{Speed}{Athletics}''}, says the \gls{gm}, then calculates  the \glspl{sunGuard} Bonus.
  They have Speed~+1 and Athletics~0.

  {\sffamily ``The \glsentrylong{tn} is 8 to flee, but you'll need 11 to get away clear.''}

  One player roll the dice: \dicef{7}.
  \begin{itemize}
    \item
  ...
  So you run into the market, and the next roll will use \roll{Speed}{Empathy}.
  Gain \pgls{ep}!''}
  \item
  ...
  That changes the field, and on the next roll, he and the \glspl{sunGuard} will roll \roll{Strength}{Athletics} (pushing past cows takes muscle).
  ...
  The second, third, and fourth will have to make another roll, though the second has selected more favourable \glspl{trait} (\roll{Speed}{Empathy}) and stands a better chance of getting that Margin of 3 to get clean away.
\end{exampletext}
