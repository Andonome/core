...
A bag has \pgls{weight} of 1, and once the bag's emptied, everyone running across that path has a $\frac{1}{D6}$ chance of stepping on one, and suffering \dmg{-2}~Damage.

Increasing the \gls{weight} increases the chances, so a bag with \pgls{weight} of 3 would leave a vitriolic path with a $\frac{3}{D6}$ chance of someone stepping on \pgls{caltrop}.

Avoiding \glspl{caltrop} just requires a \roll{Wits}{Vigilance} roll at \tn[7] and \pgls{ap}.
