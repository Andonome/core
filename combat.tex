...
\paragraph{Guarding}
\label{guarding}
allows any character to move up to 1~\gls{step} to protect something --- \pgls{npc}, a doorway, anything at all.
Anyone attempting to attack, grab, or interact with that object must make \pgls{resistedaction} against the guardian, which deals no Damage.
They may defend themself (with \roll{Dexterity}{Melee}) then rush past the guardian and strike the target, or run around the guardian (with \roll{Speed}{Athletics}), or any other task which makes sense.
Iff the guardian hits, they deal Damage as usual.

If a guarded character moves, they lose the benefits of their guardian.
And anyone can attack the guardian as usual.
...
However it goes, no \gls{weapon} can be used while grappling if they have \pgls{weight} above 1.
