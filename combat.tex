...
Athletics \gls{skill}.
Standard characters (with a Speed Bonus of +0) can move 9~\glspl{step} per \gls{round} at a full-on sprint.
With Athletics~1, they could move ($3 \times 4 =$) 12~\glspl{step}.
And a character with a Speed~Bonus of +2 and Athletics~2 could move at ($5 \times 5 =$) 25~\glspl{step} in \pgls{round}.

The movement can be divided as the player chooses, so with enough \glspl{ap} it's possible to sprint towards a band of six enemies, strike one, then \gls{retreat} before the others can initiate an attack, all by spending a single \gls{ap}.%
\footnote{\Gls{bind} has nothing like an `attack of opportunity', because attacking is the same as defending, so no such rule is required.}
\emph{However}, when the enemy has enough \glspl{ap} to act, they can interrupt any movement to attack (after which the movement may continue, if the character can still move).

From the player's perspective, you can attack any \gls{npc} running next to you and past you, unless they have more \glspl{ap} than you after they move.%
\footnote{You might consider this akin to the \textit{en passant} rule in chess, or just common sense.}
...
If they beat the \gls{tn}, they can take \pgls{running} \gls{action} to move a few \glspl{step} as long as they have positive \glspl{ap}, or simply duck and remain where they are.
Characters with negative \glspl{ap} \emph{must} duck or block without moving.
...
the distance between the pursuer and prey decreases by \pgls{area}.
