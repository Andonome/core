...
Characters can move as they please, so with enough \glspl{ap} it's possible to sprint towards a band of six men, strike one, then \gls{retreat} before the others can initiate an attack.%
...
carrying the other's \glsentrytext{weight}} uses \roll{Dexterity}{Brawl} to
attack --- success means either one can make a Grapple attack.  Some animals,
with \textit{Teeth}, can grab and deal Damage, but most humanoids need to grab
first, and \emph{then} deal Damage with a grapple.
...
Damage} \label{grappling} with an opponent might consist in bites, headbutts,
twisted bones, or strangulation-by-tentacle.  However it goes, no \gls{weapon}
can be used while grappling if they have \pgls{weight} above~1.

A successful grappling roll allow someone to break free instead of dealing Damage, if they wish.
...
Characters with a shield can block instead of dodging, using \roll{Speed}{Vigilance} 
Of course, anyone close enough can just stab an archer as usual by rolling \roll[Shield's Bonus]{Dexterity}{Melee} instead of moving.
...
When a character has no space to swing a weapon, suffer a Penalty equal to the \gls{weapon}'s Damage Bonus or \gls{weight} (whichever is lowest).
...
\pgls{weight} of~1, and once the it's emptied, everyone moving across that path
has a chance to notice the caltrops with \roll{Wits}{Vigilance}.  The
\gls{tn}TN is 6 on a clean floor, increasing by +2 on grass, +2 if running, and
