...
However, wielding \pgls{weapon} in one hand comes with limitations (see `\nameref{oneHandedWeight}', \vpageref{oneHandedWeight}).

\begin{exampletext}
  \composeHumanName\ hacks at the approaching \gls{crawler} with his longsword.
  Unfortunately, the poorly-aimed attack does very little against the silent predator's thick hide.
  The \gls{crawler} swipes and grabs at him, putting him on the back-foot again and again.

  {\sffamily
    \currentName's attack put the \gls{pc} down to 0~\glspl{ap}.
    The second attack put him down to -2.
    So he makes the third attack with a -2~Penalty, and then goes down to -4~\glspl{ap}.

    If only \currentName\ had a buckler, he could defend by spending 1~\gls{ap}.
  }
\end{exampletext}

Wielding two weapons of the same kind is mostly pointless.
...
\sidepic{Roch_Hercka/conjuration_left}{}
