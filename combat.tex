...
\Gls{crossbows} fire fast, but reloading takes a few \glspl{round}, so most fights will allow people to use their \gls{crossbow} only once.
...
The character spends \pgls{ap} so that all attacks redirect to them instead of the target.
...
\subsubsection{Twilight \& Darkness}
can reduce how far someone needs to run to really escape.
Characters running through a forest at dusk only need a Margin of 2 to flee far enough that nothing can see them.
