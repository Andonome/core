...
\index{Expendable Items!Medical kits}
Medical kits can add a Bonus to the roll, and the kit is only expended if the player needs that Bonus, so a single kit can last for a long time.
...
\Glspl{guard} who cannot fight any longer usually go to work as \glspl{helper} in the \gls{healersGuild}.
...
They have a \absNum{weaponBonus}~Bonus, but cost only 1~\gls{ap}.
Once an opponent with a large weapon has spent their \glspl{ap}, a buckler user often moves in to strike with their other weapon.
...
\begin{exampletext}
  \composeHumanName\ spots a bandit in the bushes, arrow readied, bow rising.
  He twists round with his loaded crossbow,

  {\sffamily
    The player rolls \roll{Dexterity}{Projectiles} (total: +2) against the bandit's \roll{Dexterity}{Projectiles}  (+3 total, meaning \tn[10]).
  }
\end{exampletext}
...
\begin{exampletext}
  Five goblins have javelins at the ready, and the troupe decide to rush them.

  {\sffamily
    The goblins have \roll{Dexterity}{Projectiles} +2 (\tn[9]), but they're 6~\glspl{step} away (\tn[8]).
    Everyone spends \pgls{ap} and rolls \roll{Speed}{Vigilance} to move (\gls{running} usually does not need a roll, but everyone needs to resist those javelins).

    The first \gls{pc} makes the roll, the second ties, and the third fails.
  }

  \composeHumanName\ ducks under a javelin as she rushes forwards.
  \composeHumanName\ receives a javelin in his chest, but rushes onwards regardless.
  \composeHumanName\ receives a javelin to his face and stops in shock.
\end{exampletext}
...
Their Damage equals the \gls{weapon}'s normal Damage minus 2, plus the wielder's Strength.
...
But if a player insists on ballistic financial decisions, they can inflict cuts, bruises, and serious headaches on enemies; the weapon receives a -2~Penalty to hit and -2~Damage plus the usual Strength~Bonus.

\begin{exampletext}
  \composeHumanName\ takes her last throwing knife out, and it hits the \gls{crawler} with a `thunk'.

  {\sffamily
    \currentName\ has Strength~+3 and the knife grants +1~Damage, but throwing it inflicts only \dmg{2}~Damage.
  }

  The \gls{crawler} flees, but \currentName\ wants the corpse.
  She still has her \longsword\weaponName\ in hand.

  {\sffamily
    The player rolls \roll{Dexterity}{Projectiles} with a -2~Penalty at \tn[8] (the target fled 5~\glspl{step} already).
    \addtocounter{weaponDamage}{3}%
    The standard \dmg{weaponDamage}~Damage becomes
    \addtocounter{weaponDamage}{-2}%
    \dmg{weaponDamage}.
  }
\end{exampletext}
...
\makeAutoRule{falling}{Falling}{Damage equals the height in \glsfmtplural{step} + character's Strength, \roll{Speed}{Athletics} (\glsfmttext{tn}~7+ height) for half Damage.}
\index{Falling}
inflicts Damage equal to the number of \glspl{step} fallen plus the character's Strength.%
...
So a dwarf falling 4~\glspl{step} would receive 4~Damage, which converts to \dmg{0}~Damage;%
\footnote{See `\nameref{stackingDamage}' \vpageref{stackingDamage}.}
a human with Strength +2 would receive \dmg{2}~Damage, and a gnome with Strength~-2 would receive only \dmg{-2}~Damage.

Characters can try to land gracefully, spreading the impact and halving the Damage by rolling \roll{Speed}{Athletics} at \tn[7] plus the height.
That fall from 4~\glspl{step} height would have \tn[11].

Falling \emph{onto} someone inflicts exactly the same Damage as the character receives, and a successful landing-roll lets the character add their weapon's Damage.

\Gls{armour} protects the characters as usual, because any reasonable armour has a layer of padding (but does not protect anyone they fall onto).
\index{Armour!Falling}

When characters intentionally jump in a downward arc, rather than straight down, they are `Leaping' (\vpageref{leaping}).

\begin{exampletext}
  \Pgls{sunGuard} kicks \composeHumanName\ from the citadel's window, to the ground, 3~\glspl{step} below (where more \glspl{sunGuard} stand guard, unaware of the descending weight above their heads).
  ``\textit{Best make the most of this}'', he thinks, and tries to land halberd-spike-first onto \pgls{sunGuard}'s head.

  \sffamily
  The player rolls \roll{Speed}{Athletics} at \tn[10], and adds a +2 Bonus for using the halberd to land properly.
  The player rolls `13` so with +2~Strength the character receives only \dmg{-1}~Damage.
  The \gls{sunGuard} receives all of that heft, delivered through a metal spike, and takes \dmg{4}~Damage (the high roll also means \pgls{vitalShot}).
\end{exampletext}
...
\makeAutoRule{leaping}{Leaping}{\roll{Speed}{Athletics} (\glsfmttext{tn} equals double height) or Damage equals \glsfmtplural{step} fallen plus Strength.}
works just like falling (\vpageref{falling}) but the \gls{tn} to land safely equals double the \glspl{step} fallen, and a successful roll mitigates all Damage.

Each \gls{step} a character drops is \pgls{step} forwards, so leaping from a balcony 4~\glspl{step} in the air lets the character travel an extra 4~\glspl{step} forward (in addition to their regular \gls{running} action).

\begin{exampletext}
  \composeHumanName\ sees \gls{crawler} hammering through the wheat, towards two children, just 9~\glspl{step} away.
  The \gls{village} walls stand 3~\glspl{step} tall, so he leaps forwards, spear at the ready.

  {\sffamily
    The player spends \pgls{ap} to move 4~\glspl{step}, and adds 3~\glspl{step} more for leaping from 3~\glspl{step} in the air.
    That's not nearly enough, until the \gls{gm} remembers that the children can run 2~\glspl{step} and the \gls{crawler} must be right behind them.
    The total distance is 7~\glspl{step}, but he's not resisting the fall, he's resisting the upwards-facing claws of the \gls{crawler}.
    Instead of \tn[6] he has to beat \tn[11] with \roll{Speed}{Athletics}.

    The player rolls `3', the \gls{crawler} inflicts \dmg{4}~Damage, and the ground inflicts \dmg{1}.
  }
  \currentName\ falls unconscious as the \gls{crawler} takes his body back to the forest, but at least the children returned safely.
\end{exampletext}
...
Anyone can turn to face an attacker so long as they have more \glspl{ap} than the attacker.
...
\begin{exampletext}
  ...
  He hears it behind to his left when rushing through the woods, then more to his right as he runs up to some dense bushes.
