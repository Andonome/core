...
When a character has no space to swing a weapon, suffer a Penalty equal to the \gls{weapon}'s to-hit Bonus or \gls{weight} (whichever is lowest).
...
\subsubsection{\Glsfmtname{caltrop}}
are known as `\gls{wrecan}'s snowflakes', because they can cause more animosity than a stab-wound.
A bag has \pgls{weight} of 1, and once the bag's emptied, everyone running across that path to slow down and look out (with \roll{Wits}{Vigilance} at \tn[7]).

Slowing down inflicts a -1~Penalty to chasing someone.
A failed roll to notice \glspl{caltrop} (or not bothering to stop and look) means
a $\frac{1}{D6}$ chance of stepping on one, and suffering \dmg{-2}~Damage.

Increasing the \gls{weight} increases the chances to step on \pgls{caltrop} and the \gls{tn} to spot them, so a bag with \pgls{weight} of 3 would leave a vitriolic path with a $\frac{3}{D6}$ chance of someone stepping on \pgls{caltrop}, at \tn[9] to spot them.
