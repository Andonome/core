...
(the body is often worth more than the cost of the \glspl{ration} as people have better things to do with it than make food).
...
\index{Salt for \glsfmttext{harvesting}}
\index{Knives!Hunting}
\index{Expendable Items!Salt}
\index{Items, Expendable|see {Expendable Items}}
Using the wrong \gls{equipment} (like cutting off skin with a sword) inflicts a -1~Penalty,
while specialist hunting-knives or bags of salt add a +1~Bonus.%
\footnote{Players can expend the salt for a +1~Bonus \emph{after} the roll, so a single bag of salt can last a long time.}
...
\makeAutoRule{rafting}{Rafting}{go 15 miles downstream or 5 miles up with \pgls{ep}}
Rafting downriver lets people travel at three times the normal speed (15 miles per \gls{interval}).
Rafting \emph{up} a river is at normal speed (5 miles per \gls{interval}) but comes with \pgls{ep}.
\index{Rafting|see {Boating}}

Rafting along placid waters, like a loch, mean travelling at 10 miles per \gls{interval}.

Travelling by river has the advantage of ignoring any \gls{weight} rating.
...
\Glspl{guard} of rank `\gls{fodder}', and `\gls{gDigger}' are prohibited from entering towns without written authorization, or enter \glspl{village} except to protect them.
...
\index{Talismans@\glsentryname{talisman}!Identify}%
...
The rules on \glspl{ap} should resolve disagreements on who goes first and limit how often characters act; this lets everyone declare \glspl{action} whenever they want instead of asking the rules when to act.
And the rules to create a character are just `spend 50~\glspl{xp}' -- rolling for a race or name can be left to the player's discretion.%
