...
\makeRule{cryptography}{Secret Messages}{Intelligence}{Vigilance}{against the cypher's \glsentrytext{tn}}
Some cyphers are easy to write, easy to teach, and easy to crack.
Others less so, and some are none of the above.
\index{Encryption|textbf}
\index{Coded Messages|see {Encryption}}

Creating a cypher requires an \roll{Intelligence}{Stealth} roll at \gls{tn}~7 plus the cypher's level.
Writing the cypher requires an \roll{Intelligence}{Academics} roll at \gls{tn}~7 plus double the cypher's level, and reading the cypher is the same (assuming the reader already understands it).

Decrypting a cypher requires an \roll{Intelligence}{Academics} roll at \gls{tn}~7 plus triple the level.
The decryption takes one day per level (and a failed roll mean the message has been misinterpreted) while taking \pgls{restingaction} requires one week per level (but uses no guesswork, and never suggests the wrong message).

\begin{boxtable}[cYYY]
  \textbf{Level} & \textbf{Creation \Glsentrytext{tn}} & \textbf{Reading \& Writing \Glsentrytext{tn}} & \textbf{Decryption \Glsentrytext{tn}} \\
  \hline
  1   &    8  &    9  &  10 \\
  2   &    9  &   11  &  13 \\
  3   &   10  &   13  &  16 \\
...
When a character decrypts a message as \pgls{restingaction}, they have no idea how many weeks they will need -- only that the current time has been insufficient.
