...
The caster summons mist to cover a number of squares equal to the spell's level plus the caster's Wits, so a caster with Wits +2 could cover a full 3 areas with mist.
The mist impedes vision, making it easier to sneak and harder to shoot arrows.

The mist blinds people, and any combat within the uses the rules for Darkness on page \pageref{darkness} with a maximum penalty of \arabic{spelllevel} plus the caster's Intelligence Bonus.
Firing projectiles takes a penalty equal to the number of squares from the target up to a maximum of \arabic{spelllevel} plus the caster's Intelligence Bonus;
i.e. if the caster had Intelligence +1 then the penalty for firing projectiles would be -1 per square's distance up to a maximum penalty of \setcounter{list}{0}\addtocounter{list}{-1}\addtocounter{list}{-\value{spelllevel}}\arabic{list}.
...
The mage can also send out whispering messages on the wind, making their words carry inaudibly across a room, until the target is found.
The winds can carry up to ten words, and the message can wander for miles around -- up to 1 area plus 1 per Wits Bonus the mage has.
...
If the caster has access to Metamagic's first level then extending the range means the spell can travel any distance, although longer distances can make the spell rather a long-shot, with each area traversed raising the \gls{tn} by 3.

\spell{Wind Blast}{Instant}{Survival}\\
Wind can also be made to blow forward in a blast in front of the mage.
The blast spans out, affecting 2 squares plus the mage's Wits Bonus.
...
\enhancement{1}{Expansive}The caster extends the area of effect for any level \arabic{spelllevel} Aldaron spell which affects multiple squares.
Expansive spells affect a number of \textit{areas} equal to the spell's level, plus the caster's Wits Bonus.
Inside a building, this might mean mist fills every room, while in an open forest blinding light might shine for miles around.
...
\enhancement{1}{Enlarged}
\index{Spell Enhancements!Enlarged}
...
\index{Spell Enhancements!Enormous}
...
\enhancement{1}{All-Consuming}
\index{Spell Enhancements!All-Consuming}
