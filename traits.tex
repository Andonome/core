...
\Person{\npc{\glsentrysymbol{sylf}\F\Hu}{Grogfen}}%
  {{1}{0}{1}}% BODY
  {{-1}{-1}{1}}% MIND
  {
    \setcounter{Brawl}{1}
...
The Academics \gls{skill} covers a love of learning facts, some of which may one day prove useful.
\sidebox{
  \begin{boxtable}[cX]
    \hline
...
Those without Academics cannot read much more than shop-signs, slowly, and loudly; and they cannot write more than a couple of words together.
...
\begin{description}
  \item[\roll{Strength}{Academics}]
    ...
    A full auditorium can grant a +1~Bonus to the roll due to good acoustics, or a big hat could grant a +1 Bonus for drawing everyone's attention.
    \index{Hats!Big}
  \item[\roll{Dexterity}{Academics}]
    ...
    The task benefits from a magnifying globe and plenty of sambles.
  \item[\roll{Intelligence}{Academics}]
    lets characters recall facts about an area's history.

    Libraries grant a Bonus to the roll, and each library allows another roll.
    \index{Libraries}
  \item[\roll{Wits}{Academics}]
    lets people resist spells of the Mind Sphere.
    Some think this protection comes from so years of thinking in terms of abstraction, while others say that reading too much twists the mind, making it less intelligible to anyone who hasn't read the same books.
...
\begin{exampletext}
  ...
  Everyone's demoted.
  Yous are now considered `\glspl{fodder}', and can leave the swords for the new recruits.
  ...
  Minkrash sighed -- ``Could do\ldots or we could ask the \gls{warden} if she wants to eat her words''.
...
This \gls{skill} processes raw materials to prepare, create, or preserve something.
Mastering this \gls{skill} engenders the most capable chefs, farmers, tailors, and fitters.

\begin{description}
  \item[\roll{Strength}{Cultivation}]
    to erect wooden walls.
  \item[\roll{Dexterity}{Cultivation}]
    to remove the valuable \glspl{ingredient} from \pgls{monster}'s corpse.
  \item[\roll{Intelligence}{Cultivation}]
    to predict \pgls{eclipse}.
\end{description}
...
\begin{description}
  ...
  \item[\roll{Intelligence}{Deceit}]
    covers elaborate lies and ruses, such as pretending to have a high rank in a guild.
    The tasks gains Bonuses from documentation (usually forged) and insider knowledge.
  \item[\roll{Wits}{Deceit}]
...
Some \glspl{armour} can clink or squeak, inflicting a -2~Penalty to sneaking.%
\footnote{See \vpageref{armour} for individual \glspl{armour}.}
\index{Armour!Noisy}
...
\begin{description}
  ...
  \item[\roll{Intelligence}{Stealth}]
    to create a new type of encryption to send a secret message.
    \index{Encryption}
...
This \gls{skill} averts life-threatening circumstances in the green sea past the \gls{edge} -- the untamed and uncultured wilderness.
...
\begin{exampletext}
  Goutfrak had searched a local tomb, but found it already ransacked.
  Someone in the town, somehow, knew exactly the loot lay.
  She was down to her last two \glsentrylong{gp}, but this was no time to be
  mean; it was time to spend in the right place.

  Anyone with a horde of \glspl{coin} wouldn't want attention, so they'd pick an inn where
  nobody asks questions.
  She knew of three, but newcommers would only know of two, so she picked one at random.

  The inn's \gls{baker} wasn't happy with breaking such a large \gls{coin}, but eventually agreed once she said she would leave the next day.
  Her change was eighty-seven \glsentrylongpl{cp} and nine \glsentrylongpl{sp}, each of which had the face of the man with the metal hat --- `Rex Dalyus', according to the stories at the \gls{paperGuild}.
  The \glspl{coin} looked nearly new, but must have been centuries old.

  Bingo.

  The next day she did not leave the town.
  Instead, Goutfrak took a room at a nearby inn, with a view of the first, and drank away her \glspl{coin} slowly, as she waited for the tomb-raiders to return.
