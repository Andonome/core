...
This chapter only has \glspl{spell}.  For a metaphysical deep-dive into
...
\spell{Skypunch}% Name
  ...
  {Wax}% Action
  {Air}% Spheres
  {conflicting winds or dead calm}% Resist with
  {Standing at the highest point in sight and gesturing dramatically toward the sky, the caster punches holes and slices paths through low-hanging clouds. This \gls{spell} can be used to draw patterns or even write words, but on a clear day there's nothing to work with.}% Description

\spell{Soaring Pageantry}% Name
  {Detailed, Distant, Duplicated}% Enhancements
  {Wane}% Action
  {Air}% Spheres
  {high winds or an overcast sky}% Resist with
  {The caster spins a tale into the wind, coaxing clouds to form and coalesce as illustration.
  Freak weather may follow}% Description
  {
  The scenes resemble places or faces the caster knows and chooses, but like any cloud may be interpreted differently or go unrecognized by others.
  As the \gls{spell} dissolves, roll $1D6 \times 2$ to determine the weather.%
  \exRef{judgement}{Judgement}{encWeather}
}
...
\iftoggle{intro}{}{
  ...
  \spell{Impenetrable Armour}% Name
    {}% Enhancements
    {Wax}% Action
    {Earth}% Spheres
    {material type}% Resist with
    {The caster tells a suit of armour to `tighten up' and it gains +2~\gls{dr}, while the wearer suffers a -\arabic{spellPlusTwo}~Penalty to Dexterity, as the armour becomes difficult to move in}% Description
    {Each time the armour is struck, the \gls{spell}'s effects reduce by 1.

    Targeting leather armour is \tn[8] and all metal armour requires \tn[12].}
  ...
  \spell{Enhanced Armour}% Name
    {Detailed}% Enhancements
    {Wax}% Action
    {Earth}% Spheres
    {material type}% Resist with
    {The caster tells a suit of armour to `toughen up' and it gains +2~\gls{dr}}% Description
    {Each time the armour is struck, the \gls{spell}'s effects reduce by 1.
...
\spell{Ale Mist}% Name
  ...
  Water has \tn[7], ale 9, and whiskey has \tn[11].
  Targeting a sealed barrel of alcohol demands sensing the liquid somehow, but if the spell works the exploding barrel-parts deal \showDam~Damage to anyone nearby.}
...
\iftoggle{intro}{}{
  ...
  \spell{Shroom Corpse}% Name
    {Divergent}% Enhancements
    {Wax}% Action
    {Air, Fate, Earth, Water}% Spheres
    {amount of spores present}% Resist with
    {With a mushroom-based pun the caster tells a corpse to rot, and any fungal spores on it bloom instantly}% Description
    {
    If the caster dumps fungi on the corpse which can serve as \glspl{ingredient} then the corpse produces \arabic{spellPlusOne}~\glspl{ingredient}.
}
...
\iftoggle{intro}{}{
  ...
  \spell{Forest Crown}% Name
    ...
    Antler attacks have a +\arabic{spellPlusOne} Bonus}% Description
    {
  ...
  \spell{Last Garden}% Name
    {}% Enhancements
    {Wax}% Action
    {Earth, Water}% Spheres
    {soil health}% Resist with
    {The caster demands a plant to grow, and it grows enough for \arabic{spellPlusOne}~\glspl{ration} extra}% Description
    {
    This \gls{spell} depletes the soil, which raises the \gls{tn} to grow (or cast) anything upon it by \absNum{spellPlusOne} in the future.}
  ...
  \spell{Slashers}% Name
    ...
    They grant +\arabic{spellPlusOne}~Damage when using Brawl, but inflict a -\arabic{spellPlusOne}~Penalty to using weapons or other tools}% Description
    {
...
\spell{Debt}% Name
  ...
  {The \gls{witch} thinks about how much people demand from their \glspl{spell}, and after one \showOnset\ the target's next \gls{spell} reflects onto them, and they do whatever they ask from their \gls{spell}}% Description
  {%
...
\showSpells{Death2}
  \iftoggle{judgement}{%
  	Anyone with the Xenology \gls{skill} can attempt to speak with \glspl{artefact} or the \gls{undead}.
	See \textit{Judgement} (\autopageref{Xenology}) for more.
  }{}%
...
\spell{Vomit Mana}% Name
  ...
  {Anyone without \glspl{mp} gains \glspl{ep}, as the energy rips from their body.
